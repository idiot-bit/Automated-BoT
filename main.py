    ...
    try:
        ...
        if data == "delete_apk_post":
            ...
            await context.bot.edit_message_text(
                chat_id=user_id,
                message_id=preview_id,
                text="<blockquote>🗂 <b>Select the APK to delete by name:</b></blockquote>",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )
            return
        ...
        if data.startswith("delete_apk_"):
            ...
            if apk_number <= len(apk_posts):
                ...
                # Remove the deleted APK and filename
                ...
                USER_STATE[user_id]["session_filenames"] = filenames
                ...
                if not apk_posts:
                    ...
                    keyboard = [[InlineKeyboardButton("🧹 Clear All Sessions", callback_data="erase_all_session")]]
                    await context.bot.edit_message_text(
                        chat_id=user_id,
                        message_id=query.message.message_id,
                        text=f"✅ <b>Deleted:</b> <code>{safe_filename}</code>\n<b>All APKs deleted!</b>",
                        parse_mode="HTML",
                        reply_markup=InlineKeyboardMarkup(keyboard)
                    )
                    return
        
                # Build updated keyboard with remaining APKs
                keyboard = []
                for idx, name in enumerate(filenames):
                    short_name = name if len(name) <= 20 else f"{name[:17]}..."
