...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # FIRST: Handle Channel Setting (waiting_channel)
    ...
    # THEN: Other handle_text logic (ping, help, rules, etc)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == OWNER_ID and BROADCAST_STATE.get(user_id, {}).get("status") == "waiting_message":
        BROADCAST_STATE[user_id]["message"] = update.message.text.strip()
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "confirm_broadcast":
        message_to_send = BROADCAST_STATE.get(user_id, {}).get("message")
        ...
        await query.edit_message_text(
            ...
            f"Sent: {count_success} users\n"
            f"Failed: {count_failed} users",
            parse_mode="Markdown"
        )
