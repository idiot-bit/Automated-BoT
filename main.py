...
allowed_user_keyboard = ReplyKeyboardMarkup(
    keyboard=[
        [KeyboardButton("Help")],
        [KeyboardButton("Ping"), KeyboardButton("Rules")],
        [KeyboardButton("Reset"), KeyboardButton("ViewSetup")]
    ],
    resize_keyboard=True,
    one_time_keyboard=False
    )
...
async def user_viewsetup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        f"<b>📊 STATS SYNTHESIS</b>\n"
        f"<code>🔢 Total Keys     : {key_count} Injected\n"
        f"📦 APKs Processed : {apk_count} Delivered</code>\n"
        f"<pre>├───────────────────────────┤</pre>\n\n"

        f"<b>📡 ACTIVE CHANNEL</b>\n"
        f"<code>{channel}</code>\n"
        f"<pre>├───────────────────────────┤</pre>\n\n"

        f"<b>🧬 PAYLOAD TEMPLATE</b>\n"
        f"<blockquote>{caption}</blockquote>\n"
        f"<pre>└────────── END OF REPORT ──────────┘</pre>"
    )
