...
def parse_inline_buttons(button_text: str):
    """
    Converts lines like:
    Download | https://link.com
    Support | https://t.me/support
    into InlineKeyboardMarkup
    """
    ...
    for line in button_text.strip().split("\n"):
        if "|" in line:
            ...
            buttons.append(InlineKeyboardButton(text.strip(), url.strip()))
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if user_id == OWNER_ID and BROADCAST_SESSION.get(user_id, {}).get("waiting_for_message"):
            msg = update.message
            if msg.text and "|" in msg.text and msg.text.count("http") >= 1:
                # Treat it as button input
                BROADCAST_SESSION[user_id]["buttons_raw"] = msg.text
            ...
            keyboard = parse_inline_buttons(buttons_raw) if buttons_raw else None
            ...
            if msg.text:
                ...
                await msg.reply_text("❌ Unsupported message type.", parse_mode="HTML")
...
async def send_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Try to send edit message only if it was text
        try:
        ...
        for idx, uid in enumerate(user_ids, 1):  # include OWNER_ID for testing
            ...
            if progress and ((sent + failed) % 5 == 0 or (sent + failed) == total):
                try:
                    await progress.edit_text(
                        f"🚀 Sending Broadcast: {sent}/{total}",
                        parse_mode="Markdown"
                    )
                except:
                    pass
