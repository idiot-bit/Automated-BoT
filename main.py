...
async def method2_send_to_channel(user_id, context):
    try:
        ...
        for idx, file_id in enumerate(session_files, start=1):
            ...
            # Caption builder based on key style
            if key_mode == "quote":
                caption = (
        ...
        # IST time
        ist_time = datetime.now(ZoneInfo("Asia/Kolkata")).strftime("%I:%M %p")
        ...
        apk_lines = "\n".join([f"• {name}" for name in session_filenames])
        key_style = {
            "quote": "Quote Mode",
            "mono": "Mono Mode",
            "normal": "Normal"
        ...
        summary = (
            "━━━━━━━━━━━━━━━━━━━━\n"
            f"<b>📦 M2 : {len(posted_ids)} / {len(session_files)} APKs Posted</b>\n\n"
            f"<b>🗂️ APK Names:</b>\n{apk_lines}\n\n"
            f"🔑 <b>Key Used:</b> <code>{key}</code>\n"
            f"💬 <b>Key Style:</b> {key_style}\n"
            f"🕒 <b>Time:</b> {ist_time} IST\n"
            "━━━━━━━━━━━━━━━━━━━━"
        )
