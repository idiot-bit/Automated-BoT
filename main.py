    ...
    for idx, (file_id, file_name) in enumerate(zip(session_files, session_filenames), start=1):
        ...
        if idx == len(session_files):
            if "Key -" in saved_caption:
                ...
                caption = saved_caption + f"\n{key_text}"
        else:
            caption = f"<b>{file_name}</b> ({size_display})\n{key_text}"
        ...
        try:
            ...
            print(f"Error sending APK preview: {e}")

    # Updated inline keyboard (as you wrote)
    keyboard = [
