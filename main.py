...
async def method2_send_to_channel(user_id, context):
    ...
    if len(posted_ids) >= 2:
        buttons.append([
            ...
            InlineKeyboardButton("✨ Auto Last Caption", callback_data="auto_last_caption")
        ])
        buttons.append([
            InlineKeyboardButton("🔑 Last Caption Key", callback_data="last_caption_key")
        ])
...
async def auto_last_caption(user_id, context):
    ...
    for idx, file_id in enumerate(session_files, start=1):
        if idx == len(session_files):
            # Last APK - insert key into caption
            if key_mode == "quote":
            ...
            media.append(InputMediaDocument(media=file_id))  # No caption for first APKs
    ...
    # Edit Inline Keyboard after success
    buttons = [
        [InlineKeyboardButton("📄 View Last Post", url=post_link)],
        [InlineKeyboardButton("🗑️ Delete APK Post", callback_data="delete_apk_post")],
        [InlineKeyboardButton("🔙 Back to Methods", callback_data="back_to_methods")]
    ...
    await context.bot.edit_message_text(
        chat_id=user_id,
        message_id=preview_message_id,
        text="✅ <b>Auto Last Caption Successfully!</b>\n\nManage your posts below:",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(buttons)
    )
...
async def last_caption_key(user_id, context):
    ...
    await context.bot.edit_message_text(
        chat_id=user_id,
        message_id=preview_message_id,
        text="✅ <b>Last Caption Key Successfully!</b>\n\nManage your posts below:",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(buttons)
    )
    ...
    if data == "erase_all":
        try:
            await context.bot.edit_message_reply_markup(
                chat_id=user_id,
                message_id=preview_message_id,
                reply_markup=None  # This will remove the inline keyboard
            )
            await query.edit_message_text(
                text="🧹 Session Erased! Send new APKs now.",
                parse_mode="HTML"
            )
        except Exception as e:
            print(f"Failed to erase inline keyboard: {e}")
...
async def auto_handle_channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not matched_setup:
        await context.bot.send_message(
            chat_id=OWNER_ID,
            text="⚠️ *Alert!*\n➔ *No matching Auto Setup found for this Post!*\n⛔ *Processing Declined.*",
            parse_mode="Markdown"
        )
    ...
    # Cache the message
    if chat_id not in APK_CACHE:
        APK_CACHE[chat_id] = []
    APK_CACHE[chat_id].append(message)
    ...
    # Collect all cached posts
    cached_posts = APK_CACHE.pop(chat_id, [])

    # Check if the original message still exists
    try:
        await context.bot.forward_message(chat_id=OWNER_ID, from_chat_id=source_chat_id, message_id=source_message_id)
        print("✅ Original message exists after 20s.")
    except Exception:
        await context.bot.edit_message_text(
            chat_id=OWNER_ID,
    ...
    # Find documents and extract key from last caption
    for post in cached_posts:
        if post.document:
            documents.append(post.document)
            if post.caption:
                match = re.search(r'Key\s*-\s*(\S+)', post.caption)
                if match:
                    key = match.group(1)
                elif post.caption_entities:
                    for entity in post.caption_entities:
                        if entity.type == "code":
                            offset = entity.offset
                            length = entity.length
                            key = post.caption[offset:offset + length]
                            break
    ...
    # Destinations
    dest1_channel = AUTO_SETUP["setup1"].get("dest_channel", "")
    dest2_channel = AUTO_SETUP["setup2"].get("dest_channel", "")
    dest3_channel = AUTO_SETUP["setup3"].get("dest_channel", "")

    dest1_caption = AUTO_SETUP["setup1"].get("dest_caption", "")
    dest2_caption = AUTO_SETUP["setup2"].get("dest_caption", "")
    dest3_caption = AUTO_SETUP["setup3"].get("dest_caption", "")

    style1 = AUTO_SETUP["setup1"].get("style", "mono")
    style2 = AUTO_SETUP["setup2"].get("style", "mono")
    style3 = AUTO_SETUP["setup3"].get("style", "mono")

    sent_links = []
    errors = []

    # Build caption
    def build_caption(base_caption, key, style):
        if "Key -" not in base_caption:
            base_caption += "\nKey -"
        if style == "quote":
            return base_caption.replace("Key -", f"<blockquote>Key - <code>{key}</code></blockquote>")
        else:
            return base_caption.replace("Key -", f"Key - <code>{key}</code>")

    # Send individually to Auto1 and Auto2
    for doc in documents:
        file_size = doc.file_size / (1024 * 1024)  # in MB
        try:
            if 1 <= file_size <= 50:
                caption = build_caption(dest1_caption, key, style1)
                sent_msg = await context.bot.send_document(
                    chat_id=dest1_channel,
                    document=doc.file_id,
                    caption=caption,
                    parse_mode="HTML",
                    disable_notification=True
                )
                sent_links.append(f"Auto1: https://t.me/{dest1_channel.strip('@')}/{sent_msg.message_id}")

            elif 80 <= file_size <= 2048:
                caption = build_caption(dest2_caption, key, style2)
                sent_msg = await context.bot.send_document(
                    chat_id=dest2_channel,
                    document=doc.file_id,
                    caption=caption,
                    parse_mode="HTML",
                    disable_notification=True
                )
                sent_links.append(f"Auto2: https://t.me/{dest2_channel.strip('@')}/{sent_msg.message_id}")

            else:
                print(f"⚠️ File size {file_size:.2f} MB didn't match Auto1/Auto2, skipping.")
        except Exception as e:
            print(f"❌ Error sending to Auto1/2: {traceback.format_exc()}")
            errors.append(str(e))

    # Always send all to Auto3
    try:
        caption = build_caption(dest3_caption, key, style3)

        # If multiple documents, send as media group
        if len(documents) > 1:
            media_group = []
            for idx, doc in enumerate(documents):
                if idx == 0:
                    media_group.append(InputMediaDocument(media=doc.file_id, caption=caption, parse_mode="HTML"))
                else:
                    media_group.append(InputMediaDocument(media=doc.file_id))
            sent_msgs = await context.bot.send_media_group(
                chat_id=dest3_channel,
                media=media_group,
                disable_notification=True
            )
            for sent in sent_msgs:
                sent_links.append(f"Auto3: https://t.me/{dest3_channel.strip('@')}/{sent.message_id}")
        else:
            # Only 1 file
            sent_msg3 = await context.bot.send_document(
                chat_id=dest3_channel,
            ...
            sent_links.append(f"Auto3: https://t.me/{dest3_channel.strip('@')}/{sent_msg3.message_id}")

    except Exception as e:
        print(f"❌ Error sending to Auto3: {traceback.format_exc()}")
        errors.append(str(e))

    # Build owner report
    if errors:
        final_text = f"❌ *Auto {setup_number} Completed with Errors!*\n"
        for err in errors:
            final_text += f"• `{err}`\n"
    else:
        final_text = f"✅ *Auto {setup_number} Completed Successfully!*\n"
    ...
    print("✅ Final report sent to owner.")
