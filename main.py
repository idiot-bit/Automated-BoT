...
async def process_method1_apk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if match:
        ...
        # Track usage counts
        USER_STATE[user_id]["apk_posted_count"] = USER_STATE.get(user_id, {}).get("apk_posted_count", 0) + 1
...
async def process_method2_apk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === NEW FIX: If user sends more APKs instead of key, reset session ===
    ...
    # Now safely initialize session data
    ...
    # === NEW FIX: Instantly prompt for key if 3 APKs received ===
