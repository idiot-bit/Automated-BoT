...
async def auto4_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Store the APK with caption_entities for fallback key matching
    ...
    # Start the 20s countdown only once
...
async def process_auto4_delayed(context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        for apk in AUTO4_STATE["pending_apks"]:
            try:
                await context.bot.forward_message(
                    chat_id=OWNER_ID,  # forward privately to test visibility
                    from_chat_id=source_channel,
                    message_id=apk["message_id"]
                )
                valid_apks.append(apk)
            except Exception as e:
                print(f"Auto 4: Deleted or inaccessible message: {apk['message_id']} | {e}")
        ...
        if not valid_apks:
            ...
            # Then try code entity
            elif "caption_entities" in apk:
