...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        join_date_str = join_date.strftime("%d %B %Y, %I:%M %p")
        ...
        await update.message.reply_text(
            ...
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
            "</blockquote>\n"
            "<i>NOTE: System logs are being monitored.</i>",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(keyboard),
            disable_web_page_preview=True
        )
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if data == "back_to_methods":
            ...
            # Get stored time and compute join date
            first_seen = USER_DATA.get(user_key, {}).get("first_seen", int(time.time()))
            now = int(time.time())
            days_count = (now - first_seen) // 86400
