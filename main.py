...
def save_config():
    with open("config.json", "w") as f:
        json.dump({
            ...
            "auto_setup4": AUTO_SETUP4  # <-- Add this line
        }, f, indent=4)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == OWNER_ID:
        ...
        keyboard.append([InlineKeyboardButton("⚙️ Method 4", callback_data="method_4")])  # <--- New Line
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- [Method 4 Auto APK Monitoring] ---
    if AUTO_SETUP4["enabled"]:
        source_channel = AUTO_SETUP4["source_channel"]
        
        if update.message.chat and (update.message.chat.username and f"@{update.message.chat.username}" == source_channel) \
           or (str(update.message.chat_id) == source_channel):
            # This document is coming from method4 source channel
    
            if "method4_session" not in USER_STATE:
                USER_STATE["method4_session"] = {}
            ...
            USER_STATE["method4_session"][message_id] = {
                "file_id": file_id,
                "chat_id": chat_id,
                "caption": caption,
                "time": time.time(),
                "status": "waiting",  # waiting 20s guard
                "deleted": False
            }
    
            # Start a 20-second deletion monitor
            context.application.create_task(monitor_method4_deletion(chat_id, message_id, context))
    
            return  # Important: Do not proceed to method1/method2 logic
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state.get("status") == "method4_waiting_source":
        AUTO_SETUP4["source_channel"] = message_text
        USER_STATE[user_id]["status"] = "normal"
        await update.message.reply_text("✅ Source Channel saved for Method 4.")
        return
    
    if state.get("status") == "method4_waiting_dest":
        AUTO_SETUP4["dest_channel"] = message_text
        USER_STATE[user_id]["status"] = "normal"
        await update.message.reply_text("✅ Destination Channel saved for Method 4.")
        return
    
    if state.get("status") == "method4_waiting_caption":
        if not re.search(r'Key\s*-\s*', message_text, flags=re.IGNORECASE):
            await update.message.reply_text("❌ Caption must include 'Key -'. Try again.")
            return
        AUTO_SETUP4["dest_caption"] = message_text.strip()
        USER_STATE[user_id]["status"] = "normal"
        await update.message.reply_text("✅ Destination Caption saved for Method 4.")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "method_4":
        keyboard = [
            [InlineKeyboardButton("📡 SetSource", callback_data="method4_setsource"),
             InlineKeyboardButton("🎯 SetDest", callback_data="method4_setdest")],
            [InlineKeyboardButton("✍️ SetDestcaption", callback_data="method4_setcaption")],
            [InlineKeyboardButton("✅ On", callback_data="method4_on"),
             InlineKeyboardButton("⛔ Off", callback_data="method4_off")],
            [InlineKeyboardButton("👁️ View Setup", callback_data="method4_viewsetup"),
             InlineKeyboardButton("🧹 Reset Setup", callback_data="method4_resetsetup")],
            [InlineKeyboardButton("🔙 Back to Methods", callback_data="back_to_methods")]
        ]
        await query.edit_message_text(
            "⚙️ <b>Method 4 Setup Panel</b>\nManage your method 4 settings:",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return
    ...
    # --- Method 4 Setup Buttons ---

    if data == "method4_setsource":
        USER_STATE[user_id]["status"] = "method4_waiting_source"
        await query.edit_message_text("📡 Send Source Channel ID for Method 4", parse_mode="HTML")
        return
    
    if data == "method4_setdest":
        USER_STATE[user_id]["status"] = "method4_waiting_dest"
        await query.edit_message_text("🎯 Send Destination Channel ID for Method 4", parse_mode="HTML")
        return
    
    if data == "method4_setcaption":
        USER_STATE[user_id]["status"] = "method4_waiting_caption"
        await query.edit_message_text("✍️ Send Destination Caption (must contain 'Key -') for Method 4", parse_mode="HTML")
        return
    
    if data == "method4_on":
        AUTO_SETUP4["enabled"] = True
        await query.edit_message_text("✅ Method 4 has been <b>Activated!</b>", parse_mode="HTML")
        return
    
    if data == "method4_off":
        AUTO_SETUP4["enabled"] = False
        await query.edit_message_text("⛔ Method 4 has been <b>Deactivated!</b>", parse_mode="HTML")
        return
    
    if data == "method4_viewsetup":
        msg = (
            f"👁️ <b>Method 4 Setup</b>\n"
            f"📡 Source: <code>{AUTO_SETUP4['source_channel'] or '—'}</code>\n"
            f"🎯 Destination: <code>{AUTO_SETUP4['dest_channel'] or '—'}</code>\n"
            f"✍️ Caption: {'✅ Set' if AUTO_SETUP4['dest_caption'] else '❌ Not Set'}\n"
            f"⚙️ Status: {'✅ On' if AUTO_SETUP4['enabled'] else '⛔ Off'}\n"
            f"📈 Completed Posts: {AUTO_SETUP4['completed_count']}"
        )
        await query.edit_message_text(msg, parse_mode="HTML")
        return
    
    if data == "method4_resetsetup":
        AUTO_SETUP4.update({
            "source_channel": "",
            "dest_channel": "",
            "dest_caption": "",
        ...
        await query.edit_message_text("🧹 Method 4 setup <b>Reset Successfully!</b>", parse_mode="HTML")
        return
...
async def auto_handle_channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "method4_session" in USER_STATE and message_id in USER_STATE["method4_session"]:
        USER_STATE["method4_session"][message_id]["deleted"] = True
        print(f"[Method4] Deletion detected for message_id {message_id}")
    
async def monitor_method4_deletion(chat_id, message_id, context):
    try:
        await asyncio.sleep(20)

        session = USER_STATE.get("method4_session", {}).get(message_id)

        if not session or session.get("deleted", False):
            # If deleted or no session, ignore
            return

        # 20s passed safely, continue
        caption = session["caption"]
        dest_channel = AUTO_SETUP4["dest_channel"]
        dest_caption_template = AUTO_SETUP4["dest_caption"]
        ...
        if key_match:
            ...
            # Prepare caption
            final_caption = dest_caption_template.replace("Key -", f"Key - <code>{key}</code>")
            ...
            AUTO_SETUP4["completed_count"] += 1

            # Clean up session
            USER_STATE["method4_session"].pop(message_id, None)

        else:
            # No key found, wait 3 seconds for more apks
            context.application.create_task(wait_for_more_apks(chat_id, message_id, context))

    except Exception as e:
        print(f"[Method4 Deletion Monitor Error]: {e}")

async def wait_for_more_apks(chat_id, original_message_id, context):
    try:
        await asyncio.sleep(3)

        session_data = USER_STATE.get("method4_session", {})
        candidate_messages = [mid for mid, v in session_data.items()
                              if v["chat_id"] == chat_id and not v.get("deleted", False)]

        if not candidate_messages:
            return  # No other messages to check

        key = None

        # Try to find key from newer messages
        for mid in sorted(candidate_messages):
            caption = session_data[mid]["caption"]
            match = re.search(r'Key\s*-\s*(\S+)', caption)
            if match:
                key = match.group(1)
                break
        ...
        if key:
            ...
            # Post all saved apks together
            for mid in sorted(candidate_messages):
                file_id = session_data[mid]["file_id"]
            ...
            AUTO_SETUP4["completed_count"] += len(candidate_messages)

        # Clean up sessions
        for mid in candidate_messages:
            USER_STATE["method4_session"].pop(mid, None)

    except Exception as e:
        print(f"[Method4 wait_more_apks error]: {e}")
...
async def handle_message_deleted(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "method4_session" in USER_STATE and deleted_message_id in USER_STATE["method4_session"]:
        USER_STATE["method4_session"][deleted_message_id]["deleted"] = True
