        ...
        if message_text == "ping":
            ...
            if not ALLOWED_USERS:
                ...
                for index, uid in enumerate(ALLOWED_USERS, start=1):
                    ...
                    # Try get user info live if name/username missing
                    ...
                    channel = str(user.get("channel") or "—").strip("@")
                    caption = user.get("caption")
                    caption_text = f"<pre>{escape(str(caption)[:150])}</pre>" if caption else "—"

                    start_ts = state.get("start_time", time.time())
                    days_used = (datetime.now() - datetime.fromtimestamp(start_ts)).days

                    # Alltime stats fallback = 0
