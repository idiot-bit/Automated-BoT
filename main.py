...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == OWNER_ID:
        await update.message.reply_text(
            ...
            "<pre>[ CORE.MANAGEMENT ]</pre>"
            "<pre>• /start        Restart bot session\n"
            "• /ping         Check uptime & latency\n"
            "• /rules        View bot usage policy</pre>\n"
            "<pre>[ CONFIG.UPLOAD ]</pre>"
            "<pre>• /setchannelid Set target channel\n"
            "• /setcaption   Define upload caption\n"
            "• /resetcaption Clear caption setting\n"
            "• /resetchannelid Clear channel ID\n"
            "• /reset        Full data wipe</pre>\n"
            "<pre>[ ACCESS.CONTROL ]</pre>"
            "<pre>• /adduser      Grant user access\n"
            "• /removeuser   Revoke access\n"
            "• /userlist     View allowed users</pre>\n"
        ...
        await update.message.reply_text(
            "<b>🧩 USER DASHBOARD</b>\n\n"
            "<pre>[ GENERAL.COMMANDS ]</pre>"
            "<pre>• /start        Begin session\n"
            "• /ping         Bot status\n"
            "• /rules        View usage rules</pre>\n"
            "<pre>[ PERSONAL.CONFIG ]</pre>"
            "<pre>• /setchannelid Set your upload channel\n"
            "• /setcaption   Set your upload caption\n"
            "• /resetchannelid Reset your channel\n"
            "• /resetcaption Reset your caption\n"
            "• /reset        Full user reset</pre>\n"
...
async def reset(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Styled reset confirmation
    await update.message.reply_text(
        "🧹 <b>User Data Wiped</b>\n\n"
        f"<pre>[ SYSTEM > RESET.SUCCESS ]\n[✔] User ID: {user_id}\n[✖] Channel: Cleared\n[✖] Caption: Cleared</pre>\n"
...
async def ask_to_share(update: Update):
    ...
    await update.message.reply_text(
        ...
        "[🗂️] Status: APK ready for channel\n"
        "[🤖] Action Required: Confirm share</pre>\n"
        ...
        try:
            await context.bot.edit_message_text(
                ...
                text=(
                    ...
                    "[📦] APKs Received: 3+\n"
                    "[🚨] Action Required: Send your key now</pre>\n"
            ...
            try:
                await context.bot.edit_message_text(
                    ...
                    text=(
                        "🔑 <b>KEY REQUIRED</b>\n\n"
                        "<pre>[ SYSTEM > KEY.REQUEST ]\n"
                        f"[📦] APKs Ready: {len(session_files)}\n"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not BOT_ACTIVE and user_id != OWNER_ID:
        await update.message.reply_text(
            ...
            "<b>Notice:</b> Please check back later or contact admin.",
            parse_mode="HTML"
        )
        return
    ...
    if state and state.get("status") == "waiting_channel":
        ...
        try:
            ...
            await update.message.reply_text(
                "❌ <b>Can't find the channel.</b>\n\n"
                "Please make sure:\n"
                "• The channel exists\n"
                "• The bot is added to it\n"
                "• The bot has <b>admin</b> rights",
                parse_mode="HTML"
            )
            return
        ...
        if member.status not in ["administrator", "creator"]:
            await update.message.reply_text(
                ...
                "Please add the bot as an <b>admin</b> and try again.",
                parse_mode="HTML"
            )
            return
...
async def method2_convert_quote(user_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Begin hacker-style quote output
    text_lines = ["✅ <b>Key Converted to Quote Style</b>\n"]
    text_lines.append("<pre>[ SYSTEM > QUOTE.CONVERSION ]")
    ...
    for idx, file in enumerate(session_files, start=1):
        ...
        # Terminal-style line
        text_lines.append(f"<pre>[✔] {file_name:<25} | {size_str}</pre>")
        text_lines.append(f"<blockquote>Key - <code>{key}</code></blockquote>")

    text_lines.append("<b>Status:</b> Key format converted. Quoted and ready for deploy.")
    ...
    # Edit the existing preview message
    try:
        await context.bot.edit_message_text(
            chat_id=user_id,
            message_id=preview_message_id,
...
async def method2_convert_mono(user_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Build terminal-style preview text
    text_lines = [
        "✅ <b>Key Converted to <code>Normal Style</code></b>\n",
        "<pre>[ SYSTEM > KEY.REPLACEMENT ]",
        "----------------------------------------"
    ]
    ...
    for idx, file in enumerate(session_files, start=1):
        ...
        mb = file_size / (1024 * 1024)
        size_str = f"{mb:.2f} MB" if mb < 1024 else f"{mb / 1024:.2f} GB"
        text_lines.append(f"[✔] {file_name:<25} | {size_str} | Key - {key}")

    text_lines.append("----------------------------------------")
    text_lines.append("</pre>")
    text_lines.append("<b>Status:</b> Style converted. Ready to dispatch.")
...
async def method2_edit_caption(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Build clean hacker-terminal style preview
    preview_lines = [
        "✅ <b>Caption Update Confirmed</b>\n",
        "<pre>[ SYSTEM > CAPTION.PREVIEW ]",
        "----------------------------------------"
    ]
    ...
    for idx, file in enumerate(session_files, start=1):
        ...
        mb = file_size / (1024 * 1024)
        size_str = f"{mb:.2f} MB" if mb < 1024 else f"{mb / 1024:.2f} GB"
        preview_lines.append(f"[✔] {file_name:<25} | {size_str}")

    preview_lines.append("----------------------------------------")
    preview_lines.append(f"[🔑] Key Applied: {key}")
    preview_lines.append("</pre>")
    preview_lines.append("<b>Status:</b> Ready for launch. Execute when ready.")
    ...
    # Safely delete previous preview
    try:
...
async def method2_show_preview(user_id, context):
    ...
    for idx, (file_id, file_name) in enumerate(zip(session_files, session_filenames), start=1):
        ...
        try:
            ...
            if size_bytes >= 1024 * 1024 * 1024:
                size = f"{round(size_bytes / (1024 * 1024 * 1024), 2)} GB"
            else:
                size = f"{round(size_bytes / (1024 * 1024), 1)} MB"
        except Exception:
            size = "?"

        # Key block
        if idx == len(session_files):  # last file
            if "Key -" in saved_caption:
                ...
                payload = f"Key - <code>{key}</code>" if key_mode == "mono" else f"<blockquote>Key - <code>{key}</code></blockquote>"
        else:
            payload = f"Key - <code>{key}</code>" if key_mode == "mono" else f"<blockquote>Key - <code>{key}</code></blockquote>"
    ...
    try:
        await context.bot.edit_message_text(
            ...
            text=header + preview + footer,
            parse_mode="HTML",
            disable_web_page_preview=True,
            reply_markup=keyboard
        )
    except Exception as e:
        print(f"Error in showing CEO-level preview: {e}")
    ...
    if data.startswith("auto") and data.endswith("_menu"):
        ...
        await query.edit_message_text(
            text=(
                ...
                "<pre>[ SYSTEM > AUTO.SETUP ]\n"
                f"[🔧] Setup ID: {setup_num}\n"
                "[🧠] Mode: Configurable</pre>\n"
