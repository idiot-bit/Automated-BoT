...
def is_valid_auto_source(chat_id: str) -> bool:
    """Check if the message's chat ID matches any enabled auto 1–3 source."""
    for i in range(1, 4):
        setup = AUTO_SETUP.get(f"setup{i}", {})
        if setup.get("enabled") and str(chat_id) == str(setup.get("source_channel")):
            return True
    return False
...
AUTO4_STATE = {
    "pending_apks": [],
    "timer": None,
    "setup_mode": 1,  # 1 = Setup1, 2 = Setup2
    "waiting_since": None
}
...
async def auto4_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_valid_auto_source(update.effective_chat.id):
        return  # Let auto1–3 handle it

    if not doc or not doc.file_name.lower().endswith(".apk"):
        return  # Ignore non-APK files

    chat_id = str(update.effective_chat.id)
    source_channel = str(AUTO_SETUP["setup4"].get("source_channel"))

    if chat_id != source_channel:
        # Only log if this isn't matching auto 1–3 either
        if not is_valid_auto_source(chat_id):
            await context.bot.send_message(
                OWNER_ID,
                f"⚠️ Auto4 skipped: Message from `{chat_id}`, but setup4 expects `{source_channel}`",
                parse_mode="Markdown"
            )
        return

    if not AUTO_SETUP["setup4"].get("enabled", False):
        return  # Auto 4 not enabled
    ...
    AUTO4_STATE["pending_apks"].append(apk_data)

    # Start a 10-second survival check for this APK
    asyncio.create_task(check_auto4_apk_survival(apk_data, context))

async def check_auto4_apk_survival(apk, context: ContextTypes.DEFAULT_TYPE):
    await asyncio.sleep(10)
    
    try:
        await context.bot.get_chat(apk["chat_id"])  # Ensure chat exists
        await context.bot.get_message(apk["chat_id"], apk["message_id"])  # Check message survival
    except telegram.error.BadRequest as e:
        if "message to forward not found" in str(e).lower() or "message to delete not found" in str(e).lower():
            # APK message was deleted within 10 seconds — remove it from pending list
            AUTO4_STATE["pending_apks"] = [
                a for a in AUTO4_STATE["pending_apks"]
                if a["message_id"] != apk["message_id"]
            ]
            return
    except Exception as e:
        print(f"Auto4 survival check error: {e}")
        return

    # If no timer is already running, start processing
    if not AUTO4_STATE["timer"]:
        AUTO4_STATE["waiting_since"] = time.time()
        AUTO4_STATE["timer"] = asyncio.create_task(process_auto4_logic(context))

async def process_auto4_logic(context: ContextTypes.DEFAULT_TYPE):
    try:
        await asyncio.sleep(3)  # wait for all APKs
        ...
        for apk in AUTO4_STATE["pending_apks"]:
            ...
            try:
                ...
                print(f"Auto4: Message fetch failed: {e}")

        if not valid_apks:
            await context.bot.send_message(OWNER_ID, "⚠️ Auto4: All APKs deleted or timed out.")
            return

        first_apk = valid_apks[0]
        key_match = re.search(r'Key\s*-\s*(\S+)', first_apk["caption"])

        if key_match:
            # Setup 1 Success
            key = key_match.group(1)
            await send_auto4_apks([first_apk], key, context)
            AUTO_SETUP["setup4"]["completed_count"] += 1
            save_config()
            await context.bot.send_message(OWNER_ID, "✅ Auto4 Setup1: Key Found, 1 APK Sent.")
        else:
            # Setup 2
            last_with_key = None
            for apk in valid_apks:
                ...
                if match:
                    last_with_key = (apk, match.group(1))

            if last_with_key:
                apk_obj, key = last_with_key
                await send_auto4_apks(valid_apks, key, context)
                AUTO_SETUP["setup4"]["completed_count"] += 1
                save_config()
                await context.bot.send_message(OWNER_ID, f"✅ Auto4 Setup2: Key Found, {len(valid_apks)} APKs Sent.")
            else:
                await context.bot.send_message(OWNER_ID, "❌ Auto4 Setup2: No key found after waiting.")
    except Exception as e:
        await context.bot.send_message(OWNER_ID, f"⚠️ Auto4 Error: {str(e)}")
    finally:
        AUTO4_STATE.update({
            "pending_apks": [],
            "timer": None,
...
def main():
    ...
    # Auto 4 Handler — now uses simple Document filter; logic is inside the function
    app.add_handler(MessageHandler(
        filters.ChatType.CHANNEL & filters.Document.ALL,
        auto4_message_handler
    ))

    # Method 1–3 channel post handler (if used)
    app.add_handler(MessageHandler(
        filters.UpdateType.CHANNEL_POSTS,
        auto_handle_channel_post
    ))
