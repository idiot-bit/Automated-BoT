...
async def process_method2_apk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    new_task = context.application.create_task(start_method2_countdown(user_id, context))
    ...
    if preview_message_id:
        try:
            await context.bot.edit_message_text(
                ...
                text="✅ <b>All APKs reposted with updated key caption.</b>\n\nManage your posts below:",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(buttons)
            )
        except Exception as e:
    ...
    if preview_message_id:
        try:
            await context.bot.edit_message_text(
                chat_id=user_id,
                message_id=preview_message_id,
                text="✅ <b>Key applied to last APK only.</b>\n\nManage your posts below:",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(buttons)
            )
        except:
    ...
    if preview_message_id:
        try:
            await context.bot.edit_message_text(
                chat_id=user_id,
                message_id=preview_message_id,
                text="✅ <b>Only key added to last APK.</b>\n\nManage your posts below:",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(buttons)
            )
        except:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Handle early key input during Method 2 countdown
    if state.get("current_method") == "method2" and not state.get("waiting_key"):
        if state.get("session_files"):
            ...
            # Delete countdown message if present
            if state.get("countdown_msg_id"):
                try:
            ...
            await method2_back_fullmenu(user_id, context)
            return
    ...
    if state.get("waiting_key") and state.get("current_method") == "method2":
        ...
        if not key or not session_files:
            await update.message.reply_text(
                "❌ <b>Session Error!</b>\nNo key or files found. Please /reset.",
                parse_mode="HTML"
            )
            return
    
        # Save key and update state
        state["saved_key"] = key
        state["waiting_key"] = False
        state["progress_message_id"] = None
        state["quote_applied"] = False
        ...
        # Action buttons after key capture
        buttons = [
            [
        ...
        state["preview_message_id"] = sent_message.message_id
        return
...
async def method2_back_fullmenu(user_id, context):
    ...
    # Try to edit the original preview message
    if preview_message_id:
        try:
            await context.bot.edit_message_text(
                chat_id=user_id,
            ...
            if "message to edit not found" not in str(e).lower():
                raise  # If it's a different BadRequest, re-raise
    ...
    state["preview_message_id"] = sent.message_id  # Update preview message ref
