...
async def auto_handle_channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    media_group_id = message.media_group_id or message.message_id
    ...
    await asyncio.sleep(3)  # Wait 3 seconds for possible extra files

    # After 3 seconds, check collected messages
    media_messages = MEDIA_GROUP_CACHE.pop(media_group_id, [])

    if not (1 <= len(media_messages) <= 3):
        print("❌ More than 3 APKs or none. Skipping.")
        return

    print(f"✅ Collected {len(media_messages)} APKs. Starting 20s countdown...")

    # Save message IDs for deletion checking
    source_chat_id = message.chat_id
    message_ids = [m.message_id for m in media_messages]

    countdown_msg = await context.bot.send_message(
        chat_id=OWNER_ID,
        text=f"⏳ Waiting 20s to confirm APKs still exist...",
        parse_mode="Markdown"
    )
    ...
    for sec in range(19, 0, -1):
        ...
        try:
            await context.bot.edit_message_text(
                ...
                text=f"⏳ {sec}s remaining for APK check...",
                parse_mode="Markdown"
            )
        except:
            pass
    ...
    # Check if all messages still exist
    try:
        for mid in message_ids:
            await context.bot.forward_message(chat_id=OWNER_ID, from_chat_id=source_chat_id, message_id=mid)
    except Exception:
        await context.bot.edit_message_text(
            chat_id=OWNER_ID,
            message_id=countdown_msg.message_id,
            text=f"❌ APK Deleted. Processing Declined.",
            parse_mode="Markdown"
        )
        print("❌ Some APKs deleted during wait. Skipped.")
        return

    print("✅ All APKs exist after 20s. Proceeding to key capture.")

    # Now proceed to key extraction and sending
    await process_apk_sending(media_messages, context)

async def process_apk_sending(media_messages, context):
    ...
    # Try extracting key from any caption
    for msg in media_messages:
        if msg.caption:
            match = re.search(r'Key\s*-\s*(\S+)', msg.caption)
            if match:
                key = match.group(1)
                break
    ...
    if not key:
        await context.bot.send_message(
            chat_id=OWNER_ID,
            text=f"❌ Key not found. Skipping.",
            parse_mode="Markdown"
        )
        print("❌ No key found in any caption. Skipped.")
        return

    print(f"✅ Key extracted: {key}")

    # Helper function
    def build_caption(base, key, style):
        if "Key -" not in base:
            base += "\nKey -"
        if style == "quote":
            return base.replace("Key -", f"<blockquote>Key - <code>{key}</code></blockquote>")
        else:
            return base.replace("Key -", f"Key - <code>{key}</code>")

    sent_links = []

    # Process each APK individually
    for msg in media_messages:
        doc = msg.document
        if not doc:
            continue
        ...
        try:
            if 1 <= file_size <= 50:
                ...
                # Size not match Auto1/Auto2 → still include in Auto3
                pass

        except Exception as e:
            print(f"❌ Error sending individual APK: {e}")

    # Finally send all APKs to Auto3
    for msg in media_messages:
        doc = msg.document
        if not doc:
            continue
        ...
        try:
            ...
            sent3 = await context.bot.send_document(
                chat_id=dest3,
                document=doc.file_id,
                caption=caption,
                parse_mode="HTML",
            ...
            sent_links.append(f"Auto3: https://t.me/{dest3.strip('@')}/{sent3.message_id}")

        except Exception as e:
            print(f"❌ Error sending to Auto3: {e}")

    # Owner final success message
    if sent_links:
        final_text = "✅ *Auto Completed*\n\n" + "\n".join([f"🔗 {l}" for l in sent_links])
        await context.bot.send_message(
            chat_id=OWNER_ID,
            text=final_text,
            parse_mode="Markdown"
