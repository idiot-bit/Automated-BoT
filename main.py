...
async def method2_send_to_channel(user_id, context):
    ...
    if len(posted_ids) >= 2:
        ...
        buttons.append([InlineKeyboardButton("✏️ Auto Last-Caption", callback_data="auto_last_caption")])
        buttons.append([InlineKeyboardButton("✏️ Auto Last-Key", callback_data="last_caption_key")])
...
async def auto_last_caption(user_id, context):
    ...
    apk_posts = user_state.get("apk_posts", [])
    last_post_caption = USER_DATA.get(str(user_id), {}).get("caption", "")
    key = user_state.get("saved_key", "")
    channel_id = USER_DATA.get(str(user_id), {}).get("channel", "")
    key_mode = user_state.get("key_mode", "normal")

    if not apk_posts or not last_post_caption or not key:
        await context.bot.send_message(chat_id=user_id, text="⚠️ Nothing to re-caption.", parse_mode="Markdown")
        return

    session_files = user_state.get("session_files", [])
    if not session_files:
        await context.bot.send_message(chat_id=user_id, text="⚠️ No APK files found!", parse_mode="Markdown")
        return
    ...
    for idx, file_id in enumerate(session_files, start=1):
        ...
        if key_mode == "quote":
            if len(session_files) == 1 or is_last_apk:
                caption = last_post_caption.replace("Key -", f"<blockquote>Key - <code>{key}</code></blockquote>")
            else:
                caption = f"<blockquote>Key - <code>{key}</code></blockquote>"

        elif key_mode == "mono":
            if len(session_files) == 1 or is_last_apk:
                caption = last_post_caption.replace("Key -", f"Key - <code>{key}</code>")
            else:
                caption = f"Key - <code>{key}</code>"

        else:
            if len(session_files) == 1 or is_last_apk:
                caption = last_post_caption.replace("Key -", f"Key - {key}")
            else:
                caption = f"Key - {key}"
    ...
    # Send success message + Inline buttons
    buttons = [
    ...
    await context.bot.send_message(
        chat_id=user_id,
        text="✅ *Auto Last Caption Successfully!*\n\n📄 [View Post]({})".format(post_link),
        parse_mode="Markdown",
...
async def last_caption_key(user_id, context):
    ...
    if not apk_posts or not key:
        await context.bot.send_message(chat_id=user_id, text="⚠️ Nothing to send.", parse_mode="Markdown")
        return
    ...
    # Important: Session ends after posting
    USER_STATE[user_id]["session_files"] = []
    USER_STATE[user_id]["session_filenames"] = []
    USER_STATE[user_id]["saved_key"] = None
    USER_STATE[user_id]["waiting_key"] = False
...
async def erase_all_session(user_id, context):
    ...
    await context.bot.send_message(
        chat_id=user_id,
        text="🗑️ *All session APKs erased!* You can now send new APKs.",
        parse_mode="Markdown"
    )
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data == "auto_last_caption":
        await auto_last_caption(user_id, context)
        return
    
    elif data == "last_caption_key":
