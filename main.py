...
async def auto_handle_channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- SKIP Auto 4 source ---
    source_channel4 = AUTO_SETUP.get("setup4", {}).get("source_channel", "")
    if source_channel4:
        msg_chat = update.channel_post.chat
        source_username = f"@{msg_chat.username}" if msg_chat.username else str(msg_chat.id)
        if source_username == source_channel4:
