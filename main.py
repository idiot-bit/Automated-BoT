...
async def backup_config(context=None, query=None):
    ...
    zip_filename = f"bot_backup_{now.strftime('%Y-%m-%d-%H-%M')}.zip"
        ...
        try:
            with open(zip_filename, "rb") as f:
                await context.bot.send_document(
                    chat_id=OWNER_ID,
                    document=f,
                    caption=f"💀Backup Date : {date_str}\n⏳Backup Time : {time_str}"
                )
        except Exception as e:
            print(f"Failed to send backup ZIP: {e}")

    # Edit the same message with inline keyboard
    if query:
        await query.edit_message_text(
            text="✅ Full backup ZIP created and sent to your PM!",
            reply_markup=get_main_inline_keyboard()  # Use your actual keyboard function
        )
...
async def process_method2_apk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # If 3 APKs received, skip countdown and request key
...
async def schedule_stat_reports(application: Application):
    while True:
        ...
        # Monthly report: last day of month at 10 PM
        if (now.hour == 22 and now.minute == 0 and
            (now + datetime.timedelta(days=1)).day == 1):
            await send_user_stats_report(application.bot, hours=720)  # 720 = ~30 days
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message_text == "ping":
        ...
        for index, uid in enumerate(ALLOWED_USERS, start=1):
            ...
            lines.append(f"<b>👤 User {index}:</b> <code>{uid}</code>")
            lines.append(f"├─ 🔖 Name       : {name}")
            lines.append(f"├─ 🧬 Username   : @{uname}" if uname != "—" else "├─ 🧬 Username   : —")
            lines.append(f"├─ 📝 Caption    : {caption}")
            lines.append(f"├─ 📡 Channel    : {channel}")
            lines.append(f"├─ 📦 APKs Sent  : {apk_count}")
            lines.append(f"├─ 🔑 Keys Used  : {key_count}")
            lines.append(f"└─ ⚙️ Method     : {last_method}")
            lines.append("━━━━━━━━━━━━━━━━━━━━")
    
        lines.append("<pre>")
        lines.append(f"📊 Total Users Tracked: {len(ALLOWED_USERS)}")
        lines.append(f"📅 Report Generated: {datetime.now().strftime('%Y-%m-%d')}")
        lines.append("🧠 Powered by @Ceo_DarkFury")
        lines.append("</pre>")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "erase_all":
        ...
        await context.bot.send_message(chat_id=user_id, text="🧹 Session Erased! Send new APKs now.")
        return
    ...
    if data == "backup_config" and query.from_user.id == OWNER_ID:
        await backup_config(context=context, query=query)
        return
...
async def async_main():
    ...
    app.add_handler(MessageHandler(filters.ChatType.CHANNEL & filters.Document.ALL, unified_auto_handler))
    ...
    # Move task creation *after* starting app
