...
def check_auto123_source(chat_id):
    for i in range(1, 4):
        setup = AUTO_SETUP.get(f"setup{i}")
        if setup and setup.get("enabled") and str(chat_id) == str(setup.get("source_channel", "")):
            return True
    return False
...
async def auto4_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Match both @username and -100xxxxxxxxxx
    ...
    # If no timer running, start 3 sec wait
...
def auto123_filter(message):
    return check_auto123_source(message.chat_id)

def auto4_filter(message):
    return str(message.chat_id) == str(AUTO_SETUP.get("setup4", {}).get("source_channel", ""))
...
def main():
    ...
    app.add_handler(MessageHandler(
        filters.ChatType.CHANNEL & filters.Document.ALL & filters.UserDefined(auto123_filter),
        auto_handle_channel_post
    ))

    # Auto 4: Handles only its own source channel
    app.add_handler(MessageHandler(
        filters.ChatType.CHANNEL & filters.Document.ALL & filters.UserDefined(auto4_filter),
