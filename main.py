...
async def auto_handle_channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # We only have single post - so only 1 document is available here
    # In a full media group system, you would collect via cache — later if you want full media group support we can extend it.

    if not documents:
        await context.bot.edit_message_text(
            chat_id=OWNER_ID,
            message_id=countdown_msg.message_id,
            text=f"❌ *Auto {setup_number} Declined*\n➔ *No APK found in post.*",
            parse_mode="Markdown"
        )
    ...
    last_caption = message.caption or ""
    key = None
    match = re.search(r'Key\s*-\s*(\S+)', last_caption)
    if match:
        key = match.group(1)
        ...
        for entity in message.caption_entities:
            if entity.type == "code":
                ...
                key = last_caption[offset:offset + length]
                break
    ...
    for doc in documents:
        ...
        try:
            if 1 <= file_size <= 50:
                ...
                print(f"⚠️ File size {file_size:.2f} MB didn't match Auto1/Auto2. Only sending to Auto3.")

        except Exception as e:
            print(f"❌ Error sending to Auto1/Auto2:\n{traceback.format_exc()}")
            errors.append(str(e))

    # Always send to Auto 3 (group if more than 1 document later)
    try:
        ...
        print(f"❌ Error sending to Auto3:\n{traceback.format_exc()}")
        errors.append(str(e))

    # Build Owner final report
    if errors:
        final_text = f"❌ *Auto {setup_number} Partial Error!*\n"
        for err in errors:
            final_text += f"• `{err}`\n"
    else:
