...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Check user's active method session ---
...
async def auto4_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not doc or not doc.file_name.lower().endswith(".apk"):
        return  # Ignore non-APK

    chat_id = str(update.effective_chat.id)
    source_channel = str(AUTO_SETUP["setup4"].get("source_channel"))

    # Allow both numeric IDs and @username match (safely)
    chat_username = getattr(update.effective_chat, "username", None)
    if chat_id != source_channel and (f"@{chat_username}" if chat_username else None) != source_channel:
        return  # Skip unrelated channels
    ...
    await context.bot.send_message(
        OWNER_ID,
        f"Auto4: APKs captured: {len(AUTO4_STATE['pending_apks'])}. Waiting..."
    )

    # Only one timer allowed
    ...
    try:
        ...
        for apk in AUTO4_STATE["pending_apks"]:
            ...
            try:
                ...
                await context.bot.send_message(
                    OWNER_ID,
                    f"⚠️ Auto4: Skipped deleted or invalid message: {apk['message_id']}"
                )
...
async def auto_handle_channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not doc or not doc.file_name.endswith(".apk"):
        print("❌ No APK attached or invalid file. Ignored.")
        return
    ...
    # Match Setup 1, 2, or 3 only (not setup4)
    for i in range(1, 4):
        ...
        # Match using chat_id or username
        if src == chat_id:
            matched_setup = setup
            setup_number = i
            break
        elif src.startswith("@") and source_username and src.lower() == source_username.lower():
            matched_setup = setup
            setup_number = i
            break

    if not matched_setup:
        print("❌ No Auto 1–3 match. Skipped silently.")
        return  # Don't notify for unrelated channels

    if not matched_setup.get("enabled", False):
        print(f"❌ Auto {setup_number} is OFF. Ignoring.")
        return

    print(f"✅ Matched to Auto Setup {setup_number}")

    # --- Size Filter ---
    if setup_number == 1 and not (1 <= file_size_mb <= 50):
        print("❌ Auto 1 size filter failed.")
        return

    if setup_number == 2 and not (80 <= file_size_mb <= 2048):
        print("❌ Auto 2 size filter failed.")
        return
    ...
    try:
        ...
        if str(dest_channel).startswith("@"):
            ...
            post_link = f"https://t.me/c/{dest_channel[4:]}/{sent_msg.message_id}"
        else:
            post_link = "Unknown"
        ...
        await context.bot.edit_message_text(
            ...
            text=(
                ...
                f"├─ 👤 Source : `{source_username or chat_id}`\n"
                f"├─ 🎯 Destination : `{dest_channel}`\n"
                f"├─ 📡 Key : `{key}`\n"
                f"└─ 🔗 [View Post]({post_link})"
            ),
            parse_mode="Markdown",
            disable_web_page_preview=True
        )
        print("✅ Sent and logged to owner.")
