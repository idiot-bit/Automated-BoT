...
def build_terminal_report(user_id: int, scope: str, label: str) -> str:
    state = USER_STATE.get(user_id, {})
    ...
    channel = USER_DATA.get(str(user_id), {}).get("channel", "—")
    caption = "SaveD !" if USER_DATA.get(str(user_id), {}).get("caption") else "NoT !"
    ...
    return (
        ...
        f"│ STATUS      >> {status}\n"
        f"├──────────────────────┤\n"
        f"│ METHOD 1     APKs: {method1_apks} | Keys: {method1_keys}\n"
        f"│ METHOD 2     APKs: {method2_apks} | Keys: {method2_keys}\n"
        f"├──────────────────────┤\n"
        f"│ TOTAL        APKs: {total_apks} | Keys: {total_keys}\n"
        f"└────── 𝗘𝗡𝗗 𝗢𝗙 𝗥𝗘𝗣𝗢𝗥𝗧 ──────┘"
        f"</pre>"
    )
...
async def schedule_stat_reports(application: Application):
    try:
        already_sent = set()  # Prevent multiple sends within same minute
        while True:
            now = datetime.now(ZoneInfo("Asia/Kolkata"))
            ...
            async def send_scope(scope, label):
                for user_id in ALLOWED_USERS:
                    report = build_terminal_report(user_id, scope, label)
                    try:
                        await application.bot.send_message(chat_id=user_id, text=report, parse_mode="HTML")
                    except Exception as e:
            ...
            # Reset sent-tracker every new minute
...
async def test_6h(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id == OWNER_ID:
        for uid in ALLOWED_USERS:
            report = build_terminal_report(uid, "hourly", "𝟲 𝗛𝗢𝗨𝗥𝗦 𝗥𝗘𝗣𝗢𝗥𝗧")
            await context.bot.send_message(chat_id=uid, text=report, parse_mode="HTML")
        await reset_stats("hourly")

async def test_daily(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id == OWNER_ID:
        for uid in ALLOWED_USERS:
            report = build_terminal_report(uid, "daily", "𝗗𝗔𝗜𝗟𝗬 𝗥𝗘𝗣𝗢𝗥𝗧")
            await context.bot.send_message(chat_id=uid, text=report, parse_mode="HTML")
...
async def test_weekly(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id == OWNER_ID:
        for uid in ALLOWED_USERS:
            report = build_terminal_report(uid, "weekly", "𝗪𝗘𝗘𝗞𝗟𝗬 𝗥𝗘𝗣𝗢𝗥𝗧")
            await context.bot.send_message(chat_id=uid, text=report, parse_mode="HTML")
...
async def test_monthly(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id == OWNER_ID:
        for uid in ALLOWED_USERS:
            report = build_terminal_report(uid, "monthly", "𝗠𝗢𝗡𝗧𝗛𝗟𝗬 𝗥𝗘𝗣𝗢𝗥𝗧")
            await context.bot.send_message(chat_id=uid, text=report, parse_mode="HTML")
