...
last_report_times = data.get("last_report_times", {
    "6h": None,
    "24h": None,
    "168h": None,
    "720h": None
...
def load_state():
    ...
    if os.path.exists(STATE_FILE):
        with open(STATE_FILE, "r") as f:
            ...
            AUTO4_STATE = data.get("auto4_state", {})
...
def save_state():
    with open(STATE_FILE, "w") as f:
        json.dump({
            ...
            "auto4_state": AUTO4_STATE or {},
            "auto_setup": AUTO_SETUP or {},
...
def is_report_time(now: datetime, hour_list: list) -> bool:
    return now.hour in hour_list and now.minute == 0
...
allowed_user_keyboard = ReplyKeyboardMarkup(
    keyboard=[
        ...
        [KeyboardButton("Reset"), KeyboardButton("Rules")]
    ],
    resize_keyboard=True,
    one_time_keyboard=False
)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Cancel any old countdown task
    old_task = USER_STATE.get(user_id, {}).get("countdown_task")
    ...
    # Send CEO-style welcome
    await update.message.reply_text(
        "🧠 <b>Welcome to Automated Intelligence Panel™</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "🤖 <b>Status:</b> System Online & Secured\n"
        f"🙋‍♂️ <b>Hello {user.first_name}!</b> You're now interfacing with the bot panel.\n\n"
        "⚡ <b>Method 1:</b> One APK Method\n"
        "🚀 <b>Method 2:</b> 2–3 APKs Method\n\n"
    ...
    # Bonus: Notify the owner
    if user_id != OWNER_ID:
        await context.bot.send_message(
            chat_id=OWNER_ID,
            text=(
                f"📲 <b>User Restarted Session</b>\n"
                f"• Name: <b>{user.first_name}</b>\n"
                f"• ID: <a href='tg://openmessage?user_id={user_id}'>{user_id}</a>\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == OWNER_ID:
        await update.message.reply_text(
            ...
            "<b>[ CORE MANAGEMENT ]</b>\n"
            "• <b>/start</b> — Restart bot session\n"
            "• <b>/ping</b> — Check uptime & latency\n"
            "• <b>/rules</b> — View bot usage policy\n\n"
            "<b>[ CONFIG UPLOAD ]</b>\n"
            "• <b>/setchannelid</b> — Set target channel\n"
            "• <b>/setcaption</b> — Define upload caption\n"
            "• <b>/resetcaption</b> — Clear caption setting\n"
            "• <b>/resetchannelid</b> — Clear channel ID\n"
            "• <b>/reset</b> — Full data wipe\n\n"
            "<b>[ ACCESS CONTROL ]</b>\n"
            "• <b>/adduser</b> — Grant user access\n"
            "• <b>/removeuser</b> — Revoke access\n"
            "• <b>/userlist</b> — View allowed users\n\n"
            "<b>Status:</b> All systems online. You are in full command.",
            parse_mode="HTML"
        )
        ...
        await update.message.reply_text(
            ...
            "• <b>/start</b> — Begin session\n"
            "• <b>/ping</b> — Bot status\n"
            "• <b>/rules</b> — View usage rules\n\n"
            "<b>[ PERSONAL CONFIG ]</b>\n"
            "• <b>/setchannelid</b> — Set your upload channel\n"
            "• <b>/setcaption</b> — Set your upload caption\n"
            "• <b>/resetchannelid</b> — Reset your channel\n"
            "• <b>/resetcaption</b> — Reset your caption\n"
            "• <b>/reset</b> — Full user reset\n\n"
            "<b>Status:</b> Limited access. You’re operating in user mode.",
            parse_mode="HTML"
        )
...
async def reset_caption(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_authorized(user_id):
        await update.message.reply_text(
            "🛑 <b>Access Denied</b>\n"
            "<pre>[ SYSTEM > RESET.CAPTION.FAIL ]\n"
            "[🔐] You are not authorized to perform this action</pre>",
            parse_mode="HTML"
        )
        return
    ...
    save_config()
    USER_STATE[user_id]["status"] = "normal"  # Reset just in case

    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("➕ Set New Caption", callback_data="set_caption_prompt")]
    ])

    await update.message.reply_text(
        "🧼 <b>Caption Cleared</b>\n\n"
        "<pre>[ SYSTEM > RESET.CAPTION ]\n"
        "[✔] Previous caption removed\n"
        "[💡] You may now define a new one</pre>\n"
        "<b>Next:</b> Tap below to set your fresh caption.",
        parse_mode="HTML",
...
async def reset_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("➕ Set New Channel", callback_data="set_channel_prompt")]
    ])

    await update.message.reply_text(
        "📡 <b>Channel ID Cleared</b>\n\n"
        "<pre>[ SYSTEM > RESET.CHANNEL ]\n"
        "[✔] Previous Channel Removed\n"
        "[💡] Ready to assign a new one</pre>\n"
        "<b>Next:</b> Tap below to configure your channel.",
        parse_mode="HTML",
        reply_markup=keyboard
    )
...
async def reset(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply_markup = owner_keyboard if user_id == OWNER_ID else allowed_user_keyboard

    # Styled confirmation message
    await update.message.reply_text(
        "🧹 <b>User Data Wiped</b>\n\n"
        "<b>[ SYSTEM > RESET.SUCCESS ]</b>\n"
        f"<b>User ID:</b> {user_id}\n"
        "<b>Channel:</b> Cleared\n"
        "<b>Caption:</b> Cleared\n\n"
        "<b>Status:</b> Clean slate ready. Use the panel below to reconfigure.",
        parse_mode="HTML",
...
async def ask_to_share(update: Update):
    user_id = update.effective_user.id
    state = USER_STATE.get(user_id, {})
    pending = state.get("pending_apk", {})

    file_id = pending.get("file_id")
    caption = pending.get("caption", "")
    channel = pending.get("channel", "")

    # Extract Key from caption preview
    match = re.search(r"Key\s*-\s*<code>(.*?)</code>", caption)
    key = match.group(1) if match else "UNKNOWN"

    # Get APK filename and size
    try:
        file = await update.message.bot.get_file(file_id)
        file_name = getattr(update.message.document, "file_name", "APK_File.apk")
    ...
    # Inline keyboard
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🚀 Confirm Share", callback_data="share_yes"),
         InlineKeyboardButton("❌ Cancel Post", callback_data="share_no")]
    ])

    preview = (
        "📤 <b>Dispatch Confirmation</b>\n\n"
        "<pre>[ SYSTEM > POST.PREP ]\n"
        f"[📦] File       : {file_name}\n"
        f"[💾] Size       : {size_str}\n"
        f"[🔑] Key Used   : {key}\n"
        "[🤖] Status     : Ready for upload\n"
        "[🧭] Channel    : SET ✅</pre>\n\n"
        "<b>Action:</b> Do you want to post this to your channel?\n"
        "Choose wisely: 👇"
    )
...
async def process_method2_apk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    doc = update.message.document
    file_id = doc.file_id
    file_name = doc.file_name or "APK_File.apk"

    state = USER_STATE.setdefault(user_id, {})

    # Reset previous session if a key was pending
    if state.get("waiting_key"):
        ...
        if "countdown_task" in state and not state["countdown_task"].done():
            state["countdown_task"].cancel()

    # Append new file to session
    ...
    text = (
        f"✅ <b>{len(session_files)} APK(s) Received</b>\n\n"
        "<pre>[ SYSTEM > SESSION.TRACKING ]\n"
        f"[📥] Received Count : {len(session_files)}\n"
        "[⏳] Waiting 5s for next APK...</pre>"
    )
    if progress_id:
        try:
            await context.bot.edit_message_text(
    ...
    if len(session_files) >= 3 and not state.get("waiting_key") and not state.get("key_prompt_sent"):
        key_request_text = (
            "🔑 <b>KEY REQUIRED</b>\n\n"
            "<pre>[ SYSTEM > KEY.REQUEST ]\n"
            f"[📦] APKs Received : {len(session_files)}\n"
            "[⏱] Countdown     : Complete\n"
            "[🚨] Action       : Send your key now</pre>\n\n"
            "<b>Note:</b> One key applies to 2–3 APKs."
        )
        try:
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=state["progress_message_id"],
                text=key_request_text,
                parse_mode="HTML"
            )
        except:
            await context.bot.send_message(
        ...
        # Restart countdown
        existing = state.get("countdown_task")
        if existing and not existing.done():
            existing.cancel()

        state["countdown_task"] = context.application.create_task(
            countdown_and_check(user_id, chat_id, context)
        )
...
async def send_user_stats_report(context: ContextTypes.DEFAULT_TYPE, hours=6):
    ...
    for user_id in ALLOWED_USERS:
        ...
        channel = user_data.get("channel") or "NoT !"
        caption = "SaveD !" if user_data.get("caption") else "NoT !"
        key_mode = state.get("last_method", "Method 1")
        ...
        try:
            ...
            print(f"[REPORT ERROR] Failed to send report to {user_id}: {e}")
...
async def schedule_stat_reports(application: Application):
    ...
    while True:
        ...
        # 6-Hour Report
        if is_report_time(now, [2, 8, 14]) and last_report_times["6h"] != minute_key:
            await send_user_stats_report(application.bot, hours=6)
            await reset_stats(hours=6)
            last_report_times["6h"] = minute_key

        # Daily Report
        if now.hour == 20 and now.minute == 0 and last_report_times["24h"] != minute_key:
            await send_user_stats_report(application.bot, hours=24)
            await reset_stats(hours=24)
            last_report_times["24h"] = minute_key

        # Weekly Report: Sunday at 10am
        if now.weekday() == 6 and now.hour == 10 and now.minute == 0 and last_report_times["168h"] != minute_key:
            await send_user_stats_report(application.bot, hours=168)
            await reset_stats(hours=168)
            last_report_times["168h"] = minute_key

        # Monthly Report: Last day of month at 10pm
        tomorrow = now + timedelta(days=1)
        if now.hour == 22 and now.minute == 0 and tomorrow.day == 1 and last_report_times["720h"] != minute_key:
            await send_user_stats_report(application.bot, hours=720)
            await reset_stats(hours=720)
            last_report_times["720h"] = minute_key
...
async def method2_send_to_channel(user_id, context):
    ...
    if not channel_id or not saved_caption or not session_files or not key:
        await context.bot.send_message(
            chat_id=user_id,
            text="❌ <b>Missing session data!</b>\nPlease use <b>/start</b> to reinitialize.",
            parse_mode="HTML"
        )
        return
    ...
    for idx, file_id in enumerate(session_files, start=1):
        ...
        if key_mode == "quote":
            caption = saved_caption.replace("Key -", f"<blockquote>Key - <code>{key}</code></blockquote>") if is_last_apk else f"<blockquote>Key - <code>{key}</code></blockquote>"
        elif key_mode == "mono":
            caption = saved_caption.replace("Key -", f"Key - <code>{key}</code>") if is_last_apk else f"Key - <code>{key}</code>"
        else:
            caption = saved_caption.replace("Key -", f"Key - {key}") if is_last_apk else f"Key - {key}"
    ...
    if last_message:
        if channel_id.startswith("@"):
            ...
            post_link = f"https://t.me/c/{channel_id[4:]}/{last_message.message_id}"
        else:
            post_link = "Unknown"
    ...
    if len(posted_ids) >= 2:
        buttons += [
            [InlineKeyboardButton("✏️ Auto All APK Caption", callback_data="auto_recaption"),
             InlineKeyboardButton("✨ Auto Last APK Caption", callback_data="auto_last_caption")],
            [InlineKeyboardButton("🔑 Auto Last APK Only Key", callback_data="last_caption_key")]
        ]
...
async def auto_recaption(user_id, context):
    ...
    if not channel_id or not session_files or not key:
        await context.bot.send_message(
            chat_id=user_id,
            text="❌ <b>Auto-Recaption Failed</b>\n\n<pre>[ SYSTEM > ERROR ]\n[✖] Reason: Missing session data</pre>\n<b>Fix:</b> Start fresh with /start",
            parse_mode="HTML"
        )
        return
    ...
    for idx, file_id in enumerate(session_files, start=1):
        ...
        if key_mode == "quote":
            caption = saved_caption.replace("Key -", f"<blockquote>Key - <code>{key}</code></blockquote>") if is_last_apk or len(session_files) == 1 else f"<blockquote>Key - <code>{key}</code></blockquote>"
        elif key_mode == "mono":
            caption = saved_caption.replace("Key -", f"Key - <code>{key}</code>") if is_last_apk or len(session_files) == 1 else f"Key - <code>{key}</code>"
        else:
            caption = saved_caption.replace("Key -", f"Key - {key}") if is_last_apk or len(session_files) == 1 else f"Key - {key}"

        media.append(InputMediaDocument(media=file_id, caption=caption, parse_mode="HTML"))

    # Send the updated media group
    try:
        new_posts = await context.bot.send_media_group(chat_id=channel_id, media=media)
    except Exception as e:
        await context.bot.send_message(
            chat_id=user_id,
            text="❌ <b>Auto-Recaption Failed to Send</b>\n\n<pre>[ SYSTEM > POST.ERROR ]\n[✖] Telegram failed to accept the media group</pre>",
            parse_mode="HTML"
        )
        print(f"[ERROR] send_media_group failed: {e}")
        return

    # Attempt to delete old posts
    for msg_id in old_posts:
        try:
            await context.bot.delete_message(chat_id=channel_id, message_id=msg_id)
        except:
            pass  # some may already be gone

    USER_STATE[user_id]["apk_posts"] = [msg.message_id for msg in new_posts]
    last_msg = new_posts[-1]
    post_link = f"https://t.me/{channel_id.strip('@')}/{last_msg.message_id}" if channel_id.startswith("@") else f"https://t.me/c/{channel_id.replace('-100', '')}/{last_msg.message_id}"
    ...
    # Update user with styled confirmation
    try:
        await context.bot.edit_message_text(
            chat_id=user_id,
            message_id=preview_message_id,
            text=(
                "✅ <b>𝖠𝗎𝗍𝗈 𝖠𝗅𝗅 𝖠𝗉𝗄 𝖢𝖺𝗉𝗍𝗂𝗈𝗇 Completed</b>\n\n"
                "<pre>[ SYSTEM > RECAPTION.LOG ]\n"
                f"[✔] Total Updated: {len(session_files)} APKs\n"
                "[📡] New captions posted to channel\n"
                "[🧠] Mode: Auto Replace + Key Injected\n"
                "----------------------------------------</pre>\n"
                "<b>Status:</b> All entries recaptioned and deployed."
            ),
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(buttons)
        )
    except Exception as e:
        print(f"[ERROR] Failed to edit preview: {e}")
...
async def auto_last_caption(user_id, context):
    ...
    if not channel_id or not session_files or not key:
        await context.bot.send_message(
            chat_id=user_id,
            text="<b>⚠️ Session Incomplete</b>\n<pre>[ SYSTEM > MISSING.DATA ]</pre>",
            parse_mode="HTML"
        )
        return
    ...
    await context.bot.edit_message_text(
        ...
        message_id=preview_message_id,
        text=(
            "✅ <b>Auto Last APK Caption Applied</b>\n\n"
            "<pre>[ SYSTEM > RECAPTION.LAST ]\n"
            "[✔] Caption style injected on final APK\n"
            "[🚀] Media reposted to destination channel\n"
            "[🔗] Post linked below</pre>\n\n"
            "<b>Status:</b> Terminal cleared. Session closed."
        ),
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(buttons)
    )
...
async def last_caption_key(user_id, context):
    ...
    # Build media group with only last APK having key
    media = []
    for idx, file_id in enumerate(session_files, start=1):
        if idx == len(session_files):
            if key_mode == "quote":
    ...
    await context.bot.edit_message_text(
        ...
        message_id=preview_message_id,
        text=(
            "✅ <b>[ SYSTEM > AUTO.KEY.LAST ]</b>\n"
            "<pre>[✔] Caption sent only on final APK\n"
            "[🔑] Key Style: {}</pre>\n"
            "<b>Manage your posts below:</b>"
        ).format(key_mode.upper()),
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(buttons)
    )
...
async def ask_key_for_method2(user_id, context):
    ...
    await context.bot.send_message(
        chat_id=chat_id,
        text=(
            "<b>Action Required:</b> <b>𝗦𝗲𝗻𝗱 𝘆𝗼𝘂𝗿 𝗞𝗲𝘆 𝗻𝗼𝘄 ⏳</b>"
        ),
        parse_mode="HTML"
    )
...
async def settings_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("➕ Grant Access", callback_data="add_user"),
         InlineKeyboardButton("➖ Revoke Access", callback_data="remove_user")],
        [InlineKeyboardButton("👥 List Users", callback_data="view_users"),
         InlineKeyboardButton("🧾 AutoSetup List", callback_data="view_autosetup")],
        [InlineKeyboardButton("🗂 Backup Config", callback_data="backup_config")],
        [InlineKeyboardButton("♻️ Reset All Users", callback_data="force_reset")],
        [InlineKeyboardButton("🌐 Update Admin Link", callback_data="bot_admin_link")],
        [InlineKeyboardButton("🧬 Restore Backup", callback_data="backup_restore")],
        [InlineKeyboardButton("🧹 Wipe Panel Cache", callback_data="reset_settings_panel")],
        [InlineKeyboardButton("🔙 Exit Panel", callback_data="back_to_methods")]
    ]

    await update.message.reply_text(
        "<b>🛠 SYSTEM PANEL ACCESS GRANTED</b>\n\n"
        "<pre>[ CONTROL > SETTINGS_PANEL ]</pre>\n"
        "<b>Welcome Commander,</b>\n"
        "Select a task to execute from the interface below:",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def handle_backup_restore(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Validate if user is still in restore mode and within timeout
    if not state.get("awaiting_zip") or time.time() > timeout:
        return  # Session expired or invalid

    # Accept the file for restoration
    state["pending_restore_file"] = doc
    state.pop("awaiting_zip", None)
    state.pop("zip_timeout", None)

    # Show terminal-style confirmation prompt
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Confirm Restore", callback_data="confirm_restore"),
         InlineKeyboardButton("❌ Cancel Restore", callback_data="cancel_restore")],
        [InlineKeyboardButton("🔙 Return", callback_data="settings_back")]
    ])

    await update.message.reply_text(
        text=(
            "<pre>[ BACKUP.RESTORE ]</pre>\n"
            "<b>⚠️ Detected ZIP File</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            "<b>[ FILE NAME ]</b>: <code>{}</code>\n"
            "<b>[ STATUS    ]</b>: Pending confirmation\n\n"
            "<b>Proceeding will overwrite your current bot configuration!</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            "<b>Action:</b> Confirm or cancel using the options below."
...
async def handle_backup_restore_from_document(doc, context, user_id):
    try:
        ...
        # Notify user of restoration start
        status_msg = await context.bot.send_message(
            chat_id=user_id,
            text="<pre>[ RESTORE.START ]\nStatus: Extracting backup...</pre>",
            parse_mode="HTML"
        )
        ...
        try:
            ...
            await context.bot.edit_message_text(
                ...
                text=(
                    "<pre>[ RESTORE.SUCCESS ]</pre>\n"
                    "✅ <b>Backup restored successfully.</b>\n"
                    "Please restart the bot from Railway or your host."
                ),
                parse_mode="HTML"
            )
        ...
        await context.bot.send_message(
            chat_id=user_id,
            text=(
                "<pre>[ RESTORE.ERROR ]</pre>\n"
                f"❌ <b>Unexpected error during restore.</b>\n<code>{outer}</code>"
            ),
            parse_mode="HTML"
        )
...
async def countdown_and_check(user_id, chat_id, context):
    try:
        state = USER_STATE.get(user_id, {})
        already_waiting = state.get("waiting_key", False)
        already_prompted = state.get("key_prompt_sent", False)

        if already_waiting or already_prompted:
            return
            ...
            if message_id:
                try:
                    ...
                    if "Message is not modified" in str(e):
                        ...
                        # AUTO-ERASE AND SEND INLINE RECOVERY
                        await erase_all_session(user_id, context)
                        button = InlineKeyboardMarkup([
                            [InlineKeyboardButton("🧹 Start Fresh Session", callback_data="fresh_session")]
                        ])
                        await context.bot.send_message(
                            chat_id=chat_id,
                            text="❗ <b>Countdown Error Detected.</b>\n🔄 Tap below to start again.",
                            parse_mode="HTML",
                            reply_markup=button
                        )
                        return

        # Final prompt for key if still valid
        state = USER_STATE.get(user_id, {})
        session_files = state.get("session_files", [])
        ...
        if session_files and not state.get("waiting_key", False) and not state.get("key_prompt_sent", False):
            try:
                await context.bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=state.get("progress_message_id"),
                    text="<b>Action Required:</b> <b>𝗦𝗲𝗻𝗱 𝘆𝗼𝘂𝗿 𝗞𝗲𝘆 𝗻𝗼𝘄 ⏳</b>",
                    parse_mode="HTML"
                )
            except Exception:
                await context.bot.send_message(
        ...
        # Always clear countdown ref
        final_state = USER_STATE.get(user_id, {})
        if "countdown_task" in final_state:
            final_state["countdown_task"] = None
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not BOT_ACTIVE and user_id != OWNER_ID:
        await update.message.reply_text(
            "🚫 <b>Bot Offline</b>\n\n"
            "<pre>[ SYSTEM > ACCESS.DENIED ]\n"
            "[🔒] Status: Bot is currently disabled\n"
            "[👑] Only the owner may reactivate</pre>",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("📩 Contact Admin", url="https://t.me/Ceo_DarkFury")]
            ])
        )
        return
    ...
    if state and state.get("status") == "waiting_channel":
        ...
        if not (channel_id.startswith("@") or channel_id.startswith("-100")):
            await update.message.reply_text(
                ...
                "<pre>[ SYSTEM > VALIDATION.FAIL ]\n"
                "[✖] Input rejected: Invalid format\n"
                "[⚠️] Expected: Starts with @username or -100</pre>\n"
                "<b>Hint:</b> Example — @MyChannel or -1001234567890",
                parse_mode="HTML"
            )
            return
        ...
        try:
            ...
            await update.message.reply_text(
                ...
                "<b>Reason:</b> Bot can't access the channel.\n"
                "<b>Make sure:</b>\n"
                "• Channel ID is correct\n"
                "• Bot is added to the channel\n"
                "• Bot is <b>not banned</b>\n"
                "• Bot has <b>admin rights</b>",
                parse_mode="HTML"
            )
        
            # Alert the owner with clickable user ID
            await context.bot.send_message(
                chat_id=OWNER_ID,
                text=(
                    "⚠️ <b>Unauthorized Channel Attempt</b>\n\n"
                    f"User: <a href='tg://openmessage?user_id={user_id}'>User ID</a>\n"
            ...
            print(f"[SECURITY] Channel access failure: {e}")
            return
        
        if member.status not in ["administrator", "creator"]:
            await update.message.reply_text(
                "❌ <b>Bot is Not Admin</b>\n\n"
                "<b>Fix:</b> Give admin rights to the bot and try again.",
                parse_mode="HTML"
            )
            return
    
        # All OK: Save channel ID
        ...
        await update.message.reply_text(
            ...
            f"<pre>[✔] Channel Linked: {channel_id}\n[🧠] User ID: {user_id}</pre>\n\n"
            "👋 <b>Welcome!</b> Please choose your MethoD:\n\n"
            "⚡ <b>Method 1</b> — One apk MethoD\n"
            "🚀 <b>Method 2</b> — 2 - 3 apk MethoD",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return
    ...
    if state.get("status") == "waiting_caption":
        ...
        if "Key -" not in caption:
            await update.message.reply_text(
                "❌ <b>Invalid Caption</b>\n\n"
                "<pre>[ INPUT ERROR ]\n[✖] Missing required text: Key -</pre>\n"
                "<b>Fix:</b> Your caption must include <code>Key -</code> to continue.",
                parse_mode="HTML"
            )
            return
        ...
        # Send styled confirmation
        await update.message.reply_text(
            "✅ <b>Caption Saved Successfully!</b>\n\n"
            f"<pre>[✔] User ID: {user_id}\n[🧾] New Caption:\n\n{caption}</pre>\n\n"
            "<b>👋 Welcome!</b> Please choose your MethoD:\n\n"
            "⚡ <b>Method 1</b>: One apk MethoD\n"
            "🚀 <b>Method 2</b>: 2 - 3 apk MethoD",
            parse_mode="HTML",
    ...
    if state.get("waiting_key") and state.get("current_method") == "method2":
        ...
        sent_message = await update.message.reply_text(
            text=(
                "🔖 <b>Key Captured Successfully</b>\n\n"
                "<pre>[ SYSTEM > KEY.CAPTURED ]\n"
                "[✔] Status: Key received and saved</pre>\n"
...
async def method2_convert_quote(user_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not preview_message_id or not key or not session_files:
        await context.bot.send_message(
            chat_id=user_id,
            text="⚠️ <b>No session found!</b>",
            parse_mode="HTML"
        )
        return

    # Construct terminal-style block
    terminal_lines = [
        "[ SYSTEM > QUOTE.CONVERSION ]",
        "----------------------------------------"
    ]
    quote_blocks = []

    for idx, file in enumerate(session_files, start=1):
        file_name = file.get("file_name", f"APK_{idx}.apk")
        file_size = file.get("file_size", 0)
        mb = file_size / (1024 * 1024)
        size_str = f"{mb:.2f} MB" if mb < 1024 else f"{mb / 1024:.2f} GB"

        terminal_lines.append(f"[✔] {file_name:<25} | {size_str}")
        quote_blocks.append(f"<blockquote>Key - <code>{key}</code></blockquote>")

    preview_text = (
        "✅ <b>Key Converted to Quote Style</b>\n\n"
        f"<pre>{'\n'.join(terminal_lines)}</pre>\n"
        f"{''.join(quote_blocks)}\n"
        "<b>Status:</b> Key format converted. Quoted and ready for deploy."
    )

    # Mark the style as applied
    USER_STATE[user_id]["quote_applied"] = True

    # Build interactive buttons
    buttons = build_method2_buttons(user_id)
    ...
    try:
        ...
        print(f"[ERROR] Failed to update quote preview: {e}")
...
async def method2_convert_mono(user_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Build terminal-style lines
    terminal_lines = [
        "[ SYSTEM > KEY.REPLACEMENT ]",
        "----------------------------------------"
    ]
    ...
    for idx, file in enumerate(session_files, start=1):
        ...
        mb = file_size / (1024 * 1024)
        size_str = f"{mb:.2f} MB" if mb < 1024 else f"{mb / 1024:.2f} GB"
        terminal_lines.append(f"[✔] {file_name:<25} | {size_str} | Key - {key}")

    terminal_lines.append("----------------------------------------")

    # Final text with <pre> block
    preview_text = (
        "✅ <b>Key Converted to <code>Normal Style</code></b>\n\n"
        f"<pre>{'\n'.join(terminal_lines)}</pre>\n"
        "<b>Status:</b> Style converted. Ready to dispatch."
    )

    # Flag mono as applied
    USER_STATE[user_id]["mono_applied"] = True

    # Rebuild control buttons
    buttons = build_method2_buttons(user_id)
...
async def method2_edit_caption(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "Key -" not in new_caption:
        await update.message.reply_text(
            "❌ <b>Invalid Caption!</b>\n\nMust contain <code>Key -</code> placeholder.",
            parse_mode="HTML"
        )
        return

    # Save new caption to user data
    USER_DATA[str(user_id)] = USER_DATA.get(str(user_id), {})
    ...
    state = USER_STATE.get(user_id, {})
    preview_message_id = state.get("preview_message_id")
    key = html.escape(state.get("saved_key", ""))
    session_files = state.get("session_files", [])
    ...
    # Build terminal-style preview block
    terminal_lines = [
        "[ SYSTEM > CAPTION.PREVIEW ]",
        "----------------------------------------"
    ]

    for idx, file in enumerate(session_files, start=1):
        file_name = html.escape(file.get("file_name", f"APK_{idx}.apk"))
        file_size = file.get("file_size", 0)
        mb = file_size / (1024 * 1024)
        size_str = f"{mb:.2f} MB" if mb < 1024 else f"{mb / 1024:.2f} GB"
        terminal_lines.append(f"[✔] {file_name:<25} | {size_str}")

    terminal_lines.append(f"[🔑] Key Applied: {key}")
    terminal_lines.append("----------------------------------------")

    preview_text = (
        "✅ <b>Caption Update Confirmed</b>\n\n"
        f"<pre>{'\n'.join(terminal_lines)}</pre>\n"
        "<b>Status:</b> Ready for launch. Execute when ready."
    )
    ...
    try:
        ...
        print(f"[WARN] Failed to delete previous preview: {e}")
    ...
    try:
        ...
        print(f"[ERROR] Failed to send new preview: {e}")
        await update.message.reply_text(
            "❌ <b>Failed to send caption preview.</b>",
            parse_mode="HTML"
        )
...
async def method2_show_preview(user_id, context):
    ...
    key = html.escape(user_state.get("saved_key", ""))
    saved_caption = USER_DATA.get(str(user_id), {}).get("caption", "")
    key_mode = user_state.get("key_mode", "normal")
    preview_message_id = user_state.get("preview_message_id")
    ...
    header = (
        "🧬 <b>[ SYSTEM DIAGNOSTIC - APK MISSION BRIEF ]</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
    )
    ...
    for idx, (file_id, file_name) in enumerate(zip(session_files, session_filenames), start=1):
        ...
        try:
            ...
            size = f"{round(size_bytes / (1024 ** 3), 2)} GB" if size_bytes >= 1024 ** 3 else f"{round(size_bytes / (1024 ** 2), 1)} MB"
        except Exception:
            size = "?"
        ...
        block = (
            ...
            f"│ FILE        : {file_name}\n"
            f"│ SIZE        : {size}\n"
            f"│ KEY MODE    : {key_mode.upper()}\n"
            f"└──────────────────────────────────────────┘"
            "</pre>"
        )

        # Determine final payload for the last APK or for all if no saved caption
        if idx == len(session_files) and "Key -" in saved_caption:
            if key_mode == "quote":
                payload = saved_caption.replace("Key -", f"<blockquote>Key - <code>{key}</code></blockquote>")
            elif key_mode == "mono":
                payload = saved_caption.replace("Key -", f"Key - <code>{key}</code>")
            else:
                payload = saved_caption.replace("Key -", f"Key - {key}")
        else:
            if key_mode == "quote":
                payload = f"<blockquote>Key - <code>{key}</code></blockquote>"
            elif key_mode == "mono":
                payload = f"<code>Key - {key}</code>"
            else:
                payload = f"Key - {key}"

        preview_blocks.append(block)
        preview_blocks.append(payload)

    footer = (
        "\n<b>🛰️ MISSION:</b> Simulate payload delivery to target channel\n"
        "<b>🧠 MODE:</b> Method 2 (Multi-APK · Single Key)"
    )

    preview_text = header + "\n".join(preview_blocks) + footer

    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔙 Return to Arsenal", callback_data="method2_back_fullmenu")]
    ])
    ...
    try:
        ...
        print(f"[ERROR] Failed to send method2 preview: {e}")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "share_yes":
        ...
        # Post the APK to the target channel
        await context.bot.send_document(
        ...
        # Terminal-style success edit on original inline message
        await query.edit_message_text(
            text=(
                "✅ <b>APK Posted Successfully!</b>\n\n"
                "<pre>[ SYSTEM > UPLOAD.DELIVERY ]\n"
                "[✔] Status: Delivered to Channel\n"
                f"[📤] Channel: {pending['channel']}</pre>\n"
                "<b>Status:</b> You can now upload another APK or change settings."
            ),
            parse_mode="HTML"
        )
    ...
    if query.data == "fresh_session":
        ...
        await query.edit_message_text(
            text=(
                "♻️ <b>Session Reset</b>\n\n"
                "<pre>[ SYSTEM > SESSION.CLEARED ]\n"
                "[🧹] Previous data wiped\n"
                "[📥] Ready to receive new APKs</pre>\n"
                "<b>Status:</b> Send your new APKs to begin."
            ),
            parse_mode="HTML"
        )
    ...
    if data == "erase_all":
        ...
        await query.edit_message_text(
            text=(
                "🧹 <b>Session Erased</b>\n\n"
                "<pre>[ SYSTEM > SESSION.WIPE ]\n"
                "[✔] Status: Cleared APK queue\n"
                "[🔄] Mode: Fresh session ready</pre>\n"
                "<b>Status:</b> Send your new APKs to begin."
            ),
            parse_mode="HTML"
        )
    ...
    if data == "erase_all_session":
        ...
        await update.callback_query.edit_message_text(
            "🧹 <b>Session Erased Successfully</b>\n\n"
            "<pre>[ SYSTEM > SESSION.CLEAR ]\n[✔] Status: Memory flushed\n[🧠] User ID: {user_id}</pre>\n"
            "<b>Status:</b> You're now operating from a clean state.",
            parse_mode="HTML"
        )
    
    if data == "set_caption_prompt":
        USER_STATE[user_id]["status"] = "waiting_caption"

        await query.edit_message_text(
            text="📝 <b>Caption Input Mode</b>\n\n"
                 "<pre>[ SYSTEM > AWAITING.INPUT ]\n"
                 "[✏] Please send your new caption text.</pre>\n\n"
                 "<b>Note:</b> Must include <code>Key -</code> where the key will be injected.",
            parse_mode="HTML"
        )
        return
    
    if data == "set_channel_prompt":
        USER_STATE[user_id]["status"] = "waiting_channel"
        
        await query.edit_message_text(
            "🔧 <b>Channel Setup</b>\n\n"
            "<pre>[ SYSTEM > AWAITING.CHANNEL ]\n"
            "[🧭] Please send your channel ID now</pre>\n\n"
            "<b>Format:</b> <code>@channelusername</code> or <code>-1001234567890</code>",
            parse_mode="HTML"
        )
        return
    ...
    if data == "back_to_manage_post":
        ...
        for idx, (file_id, file_name) in enumerate(zip(session_files, session_filenames), start=1):
            try:
                ...
                size_str = (
                    f"{round(size / (1024 ** 3), 2)} GB"
                    if size >= 1024 ** 3 else
                    f"{round(size / (1024 ** 2), 2)} MB"
                )
            except Exception:
                file_name = "[DELETED]"
                size_str = "?"
    
            apk_list += f"📦 <b>APK {idx}</b>: {file_name} | <i>{size_str}</i>\n"
    
        buttons = [
            [InlineKeyboardButton("📄 View Last Post", url=state.get("last_post_link", "https://t.me"))],
            [InlineKeyboardButton("🗑️ Delete APK Post", callback_data="delete_apk_post")],
            [InlineKeyboardButton("🔙 Back to Methods", callback_data="back_to_methods")]
        ]
        ...
        await context.bot.edit_message_text(
            ...
            message_id=state.get("preview_message_id"),
            text=(
                f"✅ <b>Manage Your Posted APKs</b>\n\n"
                f"<b>Total APKs:</b> {apk_count}\n\n"
                f"{apk_list or 'No APKs found in this session.'}"
            ),
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(buttons)
        )
        ...
        if apk_number <= len(apk_posts):
            ...
            if not apk_posts:
                ...
                await context.bot.edit_message_text(
                    ...
                    text=(
                        "🗑️ <b>All APKs Deleted</b>\n\n"
                        "<pre>[ SYSTEM > SESSION.WIPE ]\n"
                        "[✔] Status: Cleared APK queue\n"
                        "[🔄] Mode: New session started</pre>\n"
                        "<b>Status:</b> You're ready for a fresh batch."
                    ),
                    parse_mode="HTML"
                )
                return
            ...
            await context.bot.edit_message_text(
                ...
                message_id=query.message.message_id,
                text=(
                    f"🗑️ <b>APK {apk_number} Deleted Successfully</b>\n\n"
                    "<pre>[ Channel > apk_REMOVE ]\n"
                    f"[✔] Target: APK {apk_number}\n"
                    "[⚙️] Status: Entry removed from queue</pre>\n"
    ...
    if data == "method2_back_fullmenu":
        ...
        if not preview_message_id or not key or not session_files:
            await query.edit_message_text(
                text=(
                    ...
                    "<pre>[ SYSTEM > SESSION.INVALID ]\n"
                    "[✖] Status: Expired or missing data</pre>\n"
                    "<b>Action:</b> Please restart Method 2."
                ),
                parse_mode="HTML"
            )
            return
        ...
        text = (
            "🔖 <b>Key Captured Successfully</b>\n\n"
            "<pre>[ SYSTEM > MENU.RESTORE ]\n"
            "[✔] Session is active\n"
            "[🧠] Mode: Method 2 (Multi-APK)</pre>\n"
            "<b>What would you like to do next?</b>"
        )
    ...
    if data.startswith("auto") and data.endswith("_menu"):
        ...
        await query.edit_message_text(
            text=(
                f"⚙️ <b>Auto {setup_num} Configuration Panel</b>\n\n"
                f"<b>[ SYSTEM > AUTO.SETUP ]</b>\n"
                f"<b>[🔧] Setup ID:</b> {setup_num}\n"
                "<b>[🧠] Mode:</b> Configurable\n\n"
