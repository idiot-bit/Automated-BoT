...
async def handle_backup_restore(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not doc.file_name.endswith(".zip"):
        await update.message.reply_text("❌ Invalid file. Only ZIP backups are accepted.")
        return

    file_path = await doc.get_file()
    await update.message.reply_text("⏳ Restoring backup...")

    zip_name = f"/tmp/{doc.file_name}"
    await file_path.download_to_drive(zip_name)

    try:
        with zipfile.ZipFile(zip_name, "r") as zip_ref:
            zip_ref.extractall(".")
        await update.message.reply_text("✅ Backup restored! Please restart the bot.")
    except Exception as e:
        await update.message.reply_text(f"❌ Failed to restore backup.\nError: {e}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_authorized(user_id):
        await update.message.reply_text(
            ...
            "📩 <b>Request Access:</b> <a href='https://t.me/CeoDarkFury'>@Ceo_DarkFury</a>\n"
            "🛡️ <i>Your activity has been logged.</i>\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            "🧠 <b>Secure Systems by:</b> <a href='https://t.me/CeoDarkFury'>@Ceo_DarkFury</a>",
            parse_mode="HTML",
    ...
    await update.message.reply_text(
        ...
        "🔗 <b>Powered by:</b> <a href='https://t.me/CeoDarkFury'>@Ceo_DarkFury</a>",
        parse_mode="HTML",
        disable_web_page_preview=True,
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not file_name.lower().endswith(".apk"):
        await message.reply_text(
            ...
            f"🧠 <b>Powered & Secured by:</b> <a href='https://t.me/CeoDarkFury'>@Ceo_DarkFury</a>",
            parse_mode="HTML",
            disable_web_page_preview=True
        )
        return
...
async def settings_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🧬 Backup Restore", callback_data="backup_restore")],  # ← Add this
...
async def handle_settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "view_users":
        ...
        await query.edit_message_text("📁 Please upload your backup ZIP file now.\nOnly valid `.zip` files will be accepted.")
        return
...
async def handle_backup_restore_from_document(doc, context, user_id):
    ...
    try:
        with zipfile.ZipFile(zip_path, "r") as zip_ref:
            zip_ref.extractall(".")
        await context.bot.send_message(user_id, "✅ Backup restored! Please restart the bot on Railway.")
    except Exception as e:
        await context.bot.send_message(user_id, f"❌ Restore failed.\nError: {e}")
