...
async def method2_send_to_channel(user_id, context):
    try:
        ...
        state["apk_posts"] = posted_ids  # Required for delete panel to work
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if data == "delete_apk_post":
            ...
            keyboard = []
            for idx, msg_id in enumerate(apk_posts):
                name = filenames[idx] if idx < len(filenames) else f"APK {idx+1}"
                keyboard.append([InlineKeyboardButton(f"🗑️ Delete: {name}", callback_data=f"delete_apk_{idx+1}")])
            ...
            await context.bot.edit_message_text(
                chat_id=user_id,
                message_id=preview_id,
                text="<blockquote>🗂 <b>Select the APK file you want to delete from the post list.</b></blockquote>",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )
            return
        ...
        if data.startswith("delete_apk_"):
            ...
            if not channel_id or apk_number > len(apk_posts):
                await query.answer("❌ Invalid channel or APK.", show_alert=True)
                return
        
            # Safely get filename (optional)
            deleted_name = filenames[apk_number - 1] if apk_number - 1 < len(filenames) else f"APK {apk_number}"
            msg_id = apk_posts[apk_number - 1]
        
            # Try deleting the message from the channel
            try:
                await context.bot.delete_message(chat_id=channel_id, message_id=msg_id)
            except Exception as e:
                print(f"Delete failed: {e}")
                await query.answer("❌ Failed to delete from channel.", show_alert=True)
                return
        
            # Remove deleted entry safely
            if apk_number - 1 < len(apk_posts):
                apk_posts[apk_number - 1] = None
            if apk_number - 1 < len(filenames):
            ...
            state["session_filenames"] = filenames
        
            if "last_post_session" in state:
                state["last_post_session"]["post_message_ids"] = apk_posts
        
            if not apk_posts:
                # Clear session state fully
                for key in ["session_files", "saved_key", "last_apk_time", "preview_message_id"]:
                    state[key] = None
                state["waiting_key"] = False
                state["apk_posts"] = []
                state["session_filenames"] = []
                ...
                await context.bot.edit_message_text(
                    ...
                    text=f"✅ <b>Deleted:</b> <code>{deleted_name}</code>\n✅ All APKs deleted! New session started.",
                    parse_mode="HTML"
                )
                return
        
            # Show updated delete buttons
            keyboard = []
            for idx, msg_id in enumerate(apk_posts):
                name = filenames[idx] if idx < len(filenames) else f"APK {idx+1}"
                keyboard.append([InlineKeyboardButton(f"🗑️ Delete: {name}", callback_data=f"delete_apk_{idx+1}")])
