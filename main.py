...
async def handle_method4_apk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.channel_post and str(update.channel_post.chat.username or update.channel_post.chat.id) in source_channel:
        message = update.channel_post
...
async def wait_and_process_setup2(context, source_channel, dest_channel, dest_caption):
    ...
    for apk in apks:
        try:
            await context.bot.get_chat_member(source_channel, apk["message_id"])
    ...
    try:
        ...
        AUTO_SETUP["setup4"]["completed_count"] += 1
        save_config()
        await context.bot.send_message(owner_id, "✅ Method 4: Setup2 APKs Sent Successfully.")
    except Exception as e:
        await context.bot.send_message(owner_id, f"❌ Method 4: Setup2 Send Error: {e}")
