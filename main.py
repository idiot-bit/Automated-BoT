...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if data == "method_1":
            ...
            # ✅ Dynamic buttons based on saved data
            if channel:
                buttons.append([InlineKeyboardButton("♻️ Reset Channel", callback_data="reset_channel")])
            else:
                buttons.append([InlineKeyboardButton("📡 Set Channel", callback_data="set_channel")])
        
            if caption:
                buttons.append([InlineKeyboardButton("🗑️ Reset Caption", callback_data="reset_caption")])
            else:
                buttons.append([InlineKeyboardButton("📝 Set Caption", callback_data="set_caption")])
            ...
            await query.edit_message_text(
                "<b>╭─[ METHOD 1 SELECTED ]</b>\n"
                "<blockquote>"
                f"│ {'📡 Channel: Set' if channel else '📡 Channel: ❌ Not Set'}\n"
                f"│ {'📝 Caption: Set' if caption else '📝 Caption: ❌ Not Set'}\n"
                "│ ✅ Mode: Auto Key\n"
                "│ 📤 One APK per send\n"
                "╰────────────────────────</blockquote>",
                parse_mode="HTML",
        ...
        if data == "reset_channel":
            ...
            await query.edit_message_text(
                "<b>📡 Your Channel has been reset!</b>",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([
        ...
        if data == "reset_caption":
            ...
            await query.edit_message_text(
                "<b>📝 Your Caption has been reset!</b>",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("♻️ Reset Channel", callback_data="reset_channel")],
