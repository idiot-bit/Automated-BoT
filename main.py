...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("⏳ Fetching system status...", parse_mode="HTML")
    ...
    for elapsed in range(1, 21):
        ...
        # Build progress bar
        bar = f"[{'▰' * elapsed}{'▱' * (20 - elapsed)}]"

        # Build box-style uptime clock
        uptime_str = f"{days}D : {hours}H : {minutes}M : {seconds}S"
        box_line = f"│      {uptime_str}      │"
        box_width = len(box_line) - 2
        clock_box = (
            f"╭{'─' * box_width}╮\n"
            f"{box_line}\n"
            f"╰{'─' * box_width}╯"
...
async def rules(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_authorized(update.effective_user.id):
        await update.message.reply_text(
            ...
            "[ACCESS DENIED]\n"
            UID: {} → UNAUTHORIZED\n"
            Logging intrusion... Trace engaged.\n"
            Report sent to: @Ceo_DarkFury\n"
    ...
    await update.message.reply_text(
        "```bash\n"
        ░░░ Initializing... ░░░\n"
        [OK] SYSTEM UPLINK: STABLE\n"
        [OK] NODE ID: ALPHA-TIER-X1\n"
        [SYS] AUTH TOKEN VERIFIED\n"
        [BOOT] Loading Protocol Stack...\n"
        [LOAD] FIREWALLS ▒▒▒▒▒▒▒▒▒▒ 100%\n"
        [LOAD] LOGGERS ▒▒▒▒▒▒▒▒▒▒ 100%\n"
        [LOAD] BEHAVIOR FILTERS ▒▒▒▒▒▒▒▒▒▒ 100%\n"
        ────────────────────────────────\n"
        :: EXECUTING: /rules\n"
        \n"
        # ELITE SYSTEM PROTOCOL — MANDATORY\n"
        \n"
        [1] DO NOT SPAM\n"
        ▸ Redundant messages auto-flagged. Strike = Kick.\n"
        \n"
        [2] NO EXPLOITS\n"
        ▸ Injection attempts logged and locked.\n"
        \n"
        [3] ZERO WARNINGS\n"
        ▸ Blacklist is permanent. No appeals.\n"
        \n"
        [4] BE PROFESSIONAL\n"
        ▸ This is not a toy bot. This is command-line warfare.\n"
        ────────────────────────────────\n"
        CONTACT: t.me/Ceo_DarkFury\n"
        DEPLOYED BY: ELITE SYSTEMS INTELLIGENCE DIVISION\n"
        ```",
        parse_mode="Markdown"
    )
...
async def countdown_and_check(user_id, chat_id, context):
    try:
        ...
        # Re-check session and prompt only if still not prompted
        state = USER_STATE.get(user_id, {})
        session_files = state.get("session_files", [])
