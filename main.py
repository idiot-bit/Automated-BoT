...
async def backup_config(context=None, query=None):
    ...
    # Create ZIP file with config + state + source files
    ...
    # Inline message update (if triggered from button)
    if query:
...
async def process_method2_apk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(session_files) >= 3 and not state.get("waiting_key"):
        try:
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=state.get("progress_message_id"),
...
async def handle_settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "view_users":
        ...
        save_state()
        await query.edit_message_text(
            "♻️ <b>Force Reset Done!</b>\nAll sessions cleared.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="settings_back")]])
        )
        return

    elif data == "settings_back" or data == "back_to_methods":
        await query.edit_message_text(
            "🛠️ <b>Settings Panel</b>\nManage your bot below:",
            parse_mode="HTML",
...
async def send_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid in user_ids:
        try:
            ...
            print(f"User {uid} blocked the bot. Skipping.")
            failed += 1
        except Exception as e:
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(handle_settings_callback, pattern=r"^(view_|backup_config|force_reset|settings_back|back_to_methods)$"))
