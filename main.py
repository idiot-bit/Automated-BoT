...
async def start_method2_countdown(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg_text = (
        ...
        f"📦 <b>APK</b>: <code>{apk}</code> ({size})\n"
        f"</pre>\n\n"
        f"<b>⏳ Waiting for more APKs...</b>\n"
        f"<b>⌛ Countdown: <code>10</code> seconds</b>\n"
        f"<code>[██████████]</code>"
    )
    ...
    sent = await context.bot.send_message(chat_id, text=msg_text, parse_mode="HTML", reply_markup=keyboard)
    ...
    for sec in range(10, 0, -1):
        ...
        # If countdown already removed via confirm/cancel, exit early
        ...
        try:
            await context.bot.edit_message_text(
                ...
                text=(
                    ...
                    f"</pre>\n\n"
                    f"<b>⏳ Waiting for more APKs...</b>\n"
                    f"<b>⌛ Countdown: <code>{sec}</code> seconds</b>\n"
                    f"<code>[{bar}]</code>"
                ),
                parse_mode="HTML",
                reply_markup=keyboard
        ...
        # Exit early if 3 APKs reached
        if len(state["session_files"]) >= 3:
            break

    # Countdown done or 3 APKs collected
    try:
