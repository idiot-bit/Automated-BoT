...
async def auto_handle_channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Now decide Single or Multi APK
    ...
    try:
        ...
        if str(dest_channel).startswith("@"):
            post_link = f"https://t.me/{dest_channel.strip('@')}/{sent.message_id}"
        elif str(dest_channel).startswith("-100"):
            post_link = f"https://t.me/c/{dest_channel[4:]}/{sent.message_id}"
        else:
            post_link = "Unknown"
        ...
        source = escape(source_username) if source_username else escape(chat_id)
