...
async def method2_show_preview(user_id, context):
    ...
    preview_text = "<b>┌──[ Terminal Preview Mode ]</b>\n<pre>"
    ...
    for idx, (file_id, file_name) in enumerate(zip(session_files, session_filenames), start=1):
        ...
        # Format key based on selected mode
        if key_mode == "quote":
            key_text = f'"Key - {key}"'
        elif key_mode == "mono":
            key_text = f'Key - {key}'
        ...
        if idx == len(session_files):
            ...
            final_lines = final_caption.strip().splitlines()
            for line in final_lines:
                preview_text += f"│   {line}\n"
        else:
            preview_text += f"│   🔑 {key_text}\n"

    preview_text += "</pre>"

    # Updated terminal-style inline keyboard
    keyboard = [
