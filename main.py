...
def main():
    ...
    # Auto 1–3 Handler (only reacts to enabled source channels)
    app.add_handler(MessageHandler(
        filters.ChatType.CHANNEL & filters.Document.ALL & (lambda msg: check_auto123_source(msg.chat_id)),
        auto_handle_channel_post
    ))

    # Auto 4 Handler (isolated by its own source channel)
    app.add_handler(MessageHandler(
        filters.ChatType.CHANNEL & filters.Document.ALL & 
        (lambda msg: str(msg.chat_id) == str(AUTO_SETUP.get("setup4", {}).get("source_channel", ""))),
