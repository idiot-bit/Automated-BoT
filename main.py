...
async def method2_show_preview(user_id, context):
    ...
    target_channel = user_state.get("target_channel", "Not set")
    message_id = user_state.get("preview_message_id")  # Track last preview message ID
    ...
    if key_mode == "quote":
        ...
        styled_key = f"<code>Key - {key}</code>"
    else:
    ...
    preview_text = (
        ...
        f"📡 <b>Target Channel:</b> <code>{target_channel}</code>\n\n"
        f"{apk_text}\n\n"
        f"{styled_key}"
    )
