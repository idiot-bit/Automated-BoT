...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if data == "delete_apk_post":
            ...
            filenames = USER_STATE.get(user_id, {}).get("session_filenames", [])
            ...
            user_state = USER_STATE.setdefault(user_id, {})
            apk_posts = user_state.get("apk_posts", [])
            filenames = user_state.get("last_post_session", {}).get("filenames", [])
            channel_id = USER_DATA.get(str(user_id), {}).get("channel")

            if not apk_posts or apk_number > len(apk_posts):
            ...
            user_state["apk_posts"] = apk_posts
            user_state["last_post_session"]["filenames"] = filenames
            ...
            if not apk_posts:
                ...
                keyboard = [[InlineKeyboardButton("🔙 Back", callback_data="back_to_manage_post")]]
                await query.edit_message_text(
                    text=f"✅ Deleted <b>{filename}</b>\nAll APKs removed.",
                    parse_mode="HTML",
                    reply_markup=InlineKeyboardMarkup(keyboard)
                )
                return
            ...
            await query.edit_message_text(
                text=f"✅ Deleted <b>{filename}</b>\n🗂 Select the APK to delete by name:",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )
