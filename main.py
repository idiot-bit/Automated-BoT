...
def generate_method_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    if BOT_ADMIN_LINK:
        buttons.append([InlineKeyboardButton("👨‍💻 𝗕𝗼𝘁 𝗔𝗱𝗺𝗶𝗻", url=BOT_ADMIN_LINK)])

    # Row 2: Set/Reset Channel and Caption
    row2 = []
    if channel:
        row2.append(InlineKeyboardButton("📡 𝗥𝗲𝘀𝗲𝘁 𝗖𝗵𝗮𝗻𝗻𝗲𝗹", callback_data="reset_channel"))
    else:
        row2.append(InlineKeyboardButton("📡 𝗦𝗲𝘁 𝗖𝗵𝗮𝗻𝗻𝗲𝗹", callback_data="set_channel"))

    if caption:
        row2.append(InlineKeyboardButton("📝 𝗥𝗲𝘀𝗲𝘁 𝗖𝗮𝗽𝘁𝗶𝗼𝗻", callback_data="reset_caption"))
    else:
        row2.append(InlineKeyboardButton("📝 𝗦𝗲𝘁 𝗖𝗮𝗽𝘁𝗶𝗼𝗻", callback_data="set_caption"))
    buttons.append(row2)

    # Row 3: Send APK button only if both are set
    if channel and caption:
        if current_method == "method1":
            buttons.append([InlineKeyboardButton("📤 𝗦𝗲𝗻𝗱 𝗢𝗻𝗲 𝗔𝗣𝗞", callback_data="send_apk_method1")])
        elif current_method == "method2":
            buttons.append([InlineKeyboardButton("📤 𝗦𝗲𝗻𝗱 𝟮–𝟯 𝗔𝗣𝗞𝘀", callback_data="send_apk_method2")])

    # Row 4: Back to Methods
    buttons.append([InlineKeyboardButton("🔙 𝗕𝗮𝗰𝗸 𝘁𝗼 𝗠𝗲𝘁𝗵𝗼𝗱𝘀", callback_data="back_to_methods")])
    ...
    try:
        ...
        if data == "method_1":
            ...
            text = (
                ...
                "━━━━━━━━━━━━━━━━━━━━━━\n"
                "<b>🧠 Details:</b>\n"
                "• Auto-Key detection from your APK\n"
                "• Post one APK per session\n"
