...
def generate_method_keyboard(user_id: int):
    ...
    # Conditional buttons
    if channel:
        buttons.append([InlineKeyboardButton("📡 Reset Channel", callback_data="reset_channel")])
    else:
        buttons.append([InlineKeyboardButton("📡 Set Channel", callback_data="set_channel")])

    if caption:
        buttons.append([InlineKeyboardButton("📝 Reset Caption", callback_data="reset_caption")])
    else:
        buttons.append([InlineKeyboardButton("📝 Set Caption", callback_data="set_caption")])

    # Show send button if both are set
    if channel and caption:
        method = USER_STATE.get(user_id, {}).get("current_method")
        if method == "method1":
