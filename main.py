...
async def method2_show_preview(user_id, context):
    ...
    channel = user_state.get("target_channel", "Not set")
    ...
    # Build preview text
    preview = f"👤 <b>User ID:</b> <code>{user_id}</code>\n"
    preview += f"📡 <b>Target Channel:</b> <code>{channel}</code>\n\n"
    preview += f"📦 <b>Captured APKs:</b>\n\n"
    ...
    for idx, (file_id, file_name) in enumerate(zip(session_files, session_filenames), start=1):
        ...
        # Add entry to preview
        preview += f"🔹 <b>{file_name}</b> ({size})\n{key_text}\n\n"
    ...
    try:
        if preview_message_id:
            ...
            user_state["preview_message_id"] = sent.message_id
