...
async def method2_send_to_channel(user_id, context):
    ...
    if len(posted_ids) == 1:
        ...
        USER_STATE[user_id]["from_auto_caption"] = False
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "auto_recaption":
        USER_STATE[user_id]["from_auto_caption"] = True
    ...
    if data == "back_to_manage_post":
        state = USER_STATE.get(user_id, {})
        posted_ids = state.get("apk_posts", [])
        post_link = state.get("last_post_link", "https://t.me")
        from_auto_caption = state.get("from_auto_caption", False)
    
        buttons = [[InlineKeyboardButton("📄 View Last Post", url=post_link)]]
    
        if len(posted_ids) >= 2 and not from_auto_caption:
            buttons.append([
                InlineKeyboardButton("✏️ 𝖠𝗎𝗍𝗈 𝖠𝗅𝗅 𝖠𝗉𝗄 𝖢𝖺𝗉𝗍𝗂𝗈𝗇", callback_data="auto_recaption"),
                InlineKeyboardButton("✨ 𝖠𝗎𝗍𝗈 𝖫𝖺𝗌𝗍 𝖠𝗉𝗄 𝖢𝖺𝗉𝗍𝗂𝗈𝗇", callback_data="auto_last_caption")
            ])
