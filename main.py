...
from telegram.ext import Application, CommandHandler, MessageHandler, filters, CallbackQueryHandler, ContextTypes, BaseFilter
...
class Auto4Filter(BaseFilter):
    def filter(self, message) -> bool:
        try:
            source = str(AUTO_SETUP.get("setup4", {}).get("source_channel", ""))
            return source and str(message.chat_id) == source
        except Exception:
            return False
...
def main():
    ...
    # Auto 4 Handler – isolated by channel
    auto4_filter = Auto4Filter()
    app.add_handler(MessageHandler(
        filters.ChatType.CHANNEL & filters.Document.ALL & auto4_filter,
        auto4_message_handler
    ))

    # General channel post handler (Method 1–3: auto1/2/3)
    app.add_handler(MessageHandler(
    ...
    # Text input from owner (for setting configs)
    app.add_handler(MessageHandler(
