...
async def method2_show_preview(user_id, context, message_id):
    ...
    channel_id = user_info.get("channel", "@Not_Set")
    ...
    for idx, (file_id, file_name) in enumerate(zip(session_files, session_filenames), start=1):
        ...
        if file_size >= 1024 ** 3:
            size_display = f"{round(file_size / (1024 ** 3), 2)} GB"
        else:
            size_display = f"{round(file_size / (1024 ** 2), 2)} MB"
        ...
        preview_lines.append(f"<b>{idx}. {file_name}</b> ({size_display})\n\n{key_text}\n")
    ...
    try:
        ...
        print(f"Failed to edit preview: {e}")
