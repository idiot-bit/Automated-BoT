...
async def method2_send_to_channel(user_id, context):
    try:
        ...
        summary = (
            ...
            f"{key_line}\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "<i>𝚂𝚎𝚕𝚎𝚌𝚝 𝚊𝚗𝚢 𝚋𝚎𝚕𝚘𝚠 𝚖𝚘𝚛𝚎 𝚏𝚎𝚊𝚝𝚞𝚛𝚎𝚜 𝚑𝚎𝚛𝚎 🔖,</i>"
        )
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Handle 'back_to_manage_post' callback
        if data == "back_to_manage_post":
            user = query.from_user
            user_id = user.id
            user_key = str(user_id)
        
            # Get session filenames and key mode from USER_STATE
            session_filenames = USER_STATE[user_id].get("session_filenames", [])
            key = USER_STATE[user_id].get("saved_key", "")
            key_mode = USER_STATE[user_id].get("key_mode", "normal")
            ...
            # Update message with summary and manage post options
            buttons = [
