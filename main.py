...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if method == "method1":
        ...
        # --- Modified to trigger asking for key after 3 APKs ---
        state = USER_STATE.get(user_id, {})
        session_files = state.get("session_files", [])

        # --- If 3 APKs reached, immediately ask for key ---
        if len(session_files) >= 3 and not state.get("waiting_key", False):
            USER_STATE[user_id]["waiting_key"] = True

            await context.bot.send_message(
                chat_id=user_id,
                text="🔑 *Send the Key now!* (3 APKs received)",
                parse_mode="Markdown"
            )
            return

        # Continue with regular Method 2 logic
...
async def countdown_and_check(user_id, chat_id, context):
    try:
        ...
        # --- Check if 3 APKs are received, immediately ask for Key ---
        if len(state.get("session_files", [])) >= 3 and not state.get("waiting_key", False):
            # Send key request immediately after 3 APKs received
            try:
                if message_id:
                    await context.bot.edit_message_text(
                        chat_id=chat_id,
                ...
                print(f"Error sending Key request: {e}")

            USER_STATE[user_id]["waiting_key"] = True
            USER_STATE[user_id]["progress_message_id"] = None
            return  # Exit after asking for the key

        # --- Countdown only if less than 3 APKs received ---
        for remaining in range(5, 0, -1):
            await asyncio.sleep(1)  # Sleep 1 sec, for better accuracy

            state = USER_STATE.get(user_id, {})
            if not state:
                break

            # Check if the APK count is less than 3, and continue countdown
            if state.get("session_files") and not state.get("waiting_key", False):
                try:
                    if message_id:
                        await context.bot.edit_message_text(
                            chat_id=chat_id,
        ...
        # After countdown (5 sec done)
        state = USER_STATE.get(user_id, {})
        session_files = state.get("session_files", [])

        # Now check again if we need to ask for Key (only once)
