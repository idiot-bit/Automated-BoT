...
async def auto4_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id != source_channel or not AUTO_SETUP["setup4"].get("enabled", False):
        return  # Ignore messages not from source or Auto4 OFF
...
async def process_auto4_logic(context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Filter deleted messages (within 20 sec check)
        valid_apks = []
        now = time.time()
        for apk in AUTO4_STATE["pending_apks"]:
            try:
                msg = await context.bot.get_chat(AUTO_SETUP["setup4"]["source_channel"]).get_message(apk["message_id"])
                valid_apks.append(apk)
            except Exception:
                print(f"Auto4: Message deleted, ignoring APK {apk['file_id']}")

        if not valid_apks:
            await context.bot.send_message(OWNER_ID, "⚠️ Auto4: All APKs deleted or invalid.")
            AUTO4_STATE.update({"pending_apks": [], "timer": None, "setup_mode": 1})
            return

        first_apk = valid_apks[0]
        first_caption = first_apk["caption"]

        key_match = re.search(r'Key\s*-\s*(\S+)', first_caption)
        ...
        if key_match:
            ...
            # No key → check other APKs
            if len(valid_apks) == 2 or len(valid_apks) == 3:
                last_apk_with_key = None
                for apk in valid_apks:
                    match = re.search(r'Key\s*-\s*(\S+)', apk["caption"])
                    if match:
                        last_apk_with_key = (apk, match.group(1))

                if last_apk_with_key:
                    apk_obj, key = last_apk_with_key
                    ...
                    await context.bot.send_message(OWNER_ID, "❌ Auto4: No key found after 3 APKs.")
            else:
                await context.bot.send_message(OWNER_ID, "❌ Auto4: Not enough APKs for Setup 2.")
...
def main():
    ...
    app.add_handler(MessageHandler(filters.ChatType.CHANNEL & filters.Document.ALL, auto4_message_handler))
