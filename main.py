...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Ensure USER_STATE exists for the user
        USER_STATE.setdefault(user_id, {})
        
        # Preserve the joined_at value if it exists, otherwise initialize it
        joined_at = USER_STATE[user_id].get("joined_at", int(time.time()))
        USER_STATE[user_id]["joined_at"] = joined_at

        # Cancel any leftover countdown task if needed
        if "countdown_task" in USER_STATE[user_id]:
            task = USER_STATE[user_id].get("countdown_task")
            if task and not task.done():
                task.cancel()

        # Reset session state while preserving joined_at
        USER_STATE[user_id] = {
            "joined_at": joined_at,  # preserve the joined_at field
            "current_method": None,
        ...
        # Define the keyboard with method options
        keyboard = [
        ...
        # Send a message with method options to the user
        await update.message.reply_text(
            "<pre>"
...
async def method2_send_to_channel(user_id, context):
    try:
       ...
       # Build inline buttons
        ...
        # Section: Add Key to Uploaded APKs
        if len(posted_ids) >= 2:
            buttons.append([
...
def build_terminal_report(user_id: int, scope: str, label: str):
    ...
    all_method1_apks = state.get("all_method1_apks", 0)
    all_method1_keys = state.get("all_method1_keys", 0)
    all_method2_apks = state.get("all_method2_apks", 0)
    all_method2_keys = state.get("all_method2_keys", 0)
    all_total_apks = all_method1_apks + all_method2_apks
    all_total_keys = all_method1_keys + all_method2_keys
    ...
    joined_ts = state.get("joined_time", state.get("last_used_time", time.time()))
    joined_days = max(1, int((time.time() - joined_ts) // 86400))
    ...
    if scope == "manual":
        text += (
            f"\n\n⏳ Days Used: <b>{joined_days} Days</b>\n"
            f"🗂️ All-Time: {all_total_apks} Apks, {all_total_keys} Keys"
        )

    # Timestamp footer
    text += f"\n\n🕒 Generated: {timestamp}</b></blockquote>"
...
async def reset_stats(scope: str):
    ...
    print(f"[RESET] Cleared {scope} stats at {datetime.now(ZoneInfo('Asia/Kolkata')).strftime('%H:%M:%S')}")
...
async def schedule_stat_reports(application: Application):
    try:
        ...
        while True:
            ...
            if current_minute == "08:00" and "daily" not in already_sent:
                await send_scope("daily", "𝗗𝗔𝗜𝗟𝗬 𝗥𝗘𝗣𝗢𝗥𝗧")
                already_sent.add("daily")

            if current_minute in ["02:00", "14:00", "18:00"] and current_minute not in already_sent:
                await send_scope("hourly", "𝟲 𝗛𝗢𝗨𝗥𝗦 𝗥𝗘𝗣𝗢𝗥𝗧")
...
async def user_viewsetup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text, markup = build_terminal_report(user_id, "manual", "𝗨𝗦𝗘𝗥 𝗥𝗘𝗣𝗢𝗥𝗧")
...
async def userstats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid in ALLOWED_USERS:
        text, markup = build_terminal_report(uid, "manual", "𝗨𝗦𝗘𝗥𝗦 𝗥𝗘𝗣𝗢𝗥𝗧")
        try:
            await context.bot.send_message(chat_id=user_id, text=text, parse_mode="HTML", reply_markup=markup)
        except Exception as e:
            await notify_owner_on_error(context.bot, e, source="userstats")

async def test_6h(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id == OWNER_ID:
        for uid in ALLOWED_USERS:
            text, markup = build_terminal_report(uid, "hourly", "𝟲 𝗛𝗢𝗨𝗥𝗦 𝗥𝗘𝗣𝗢𝗥𝗧")
            await context.bot.send_message(chat_id=uid, text=text, parse_mode="HTML", reply_markup=markup)
        await reset_stats("hourly")
...
def main():
    ...
    app.add_handler(CommandHandler("test6h", test_6h))
