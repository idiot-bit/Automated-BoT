...
async def process_method2_apk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Track last method usage
    state["last_method"] = "Method 2"
    state["last_style"] = state.get("key_mode", "normal")
    state["last_used_time"] = time.time()

    # Prevent spam by checking key_prompt_sent flag
    if len(session_files) >= 3 and not state.get("waiting_key", False) and not state.get("key_prompt_sent", False):
        try:
            await context.bot.edit_message_text(
                chat_id=chat_id,
