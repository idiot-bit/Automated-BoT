...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ========= Method 3 (Auto 1, 2, 3) ========= #
        ...
        USER_STATE[user_id]["status"] = "normal"
        save_config()
        await update.message.reply_text(f"✅ Source Channel saved for Auto {setup_num}!")
        ...
        USER_STATE[user_id]["status"] = "normal"
        save_config()
        await update.message.reply_text(f"✅ Destination Channel saved for Auto {setup_num}!")
        ...
        USER_STATE[user_id]["status"] = "normal"
        save_config()
        await update.message.reply_text(f"✅ Destination Caption saved for Auto {setup_num}!")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("setdestcaption"):
        ...
        await query.edit_message_text(
            f"✍️ Send Caption (must include 'Key -') for Auto {setup_num}",
            parse_mode="HTML"
        )
        return
...
async def auto_handle_channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if key_mode == "auto":
        ...
        if match:
            ...
            # Try finding any single word inside <code> tags (if present)
            html_match = re.search(r'<code>([^<>\n]+)</code>', caption)
            if html_match:
                key = html_match.group(1)
            else:
                # As last chance, find a word like 6-15 letters/numbers without Key-
                raw_key = re.search(r'\b[a-zA-Z0-9]{6,}\b', caption)
                if raw_key:
                    key = raw_key.group(0)
