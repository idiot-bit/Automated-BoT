...
async def show_broadcast_preview(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    BROADCAST_STATE[user_id]["message_object"] = update.message
    BROADCAST_STATE[user_id]["status"] = "confirm_broadcast"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # If Owner is typing a broadcast message
    if user_id == OWNER_ID and BROADCAST_STATE.get(user_id, {}).get("status") == "waiting_message":
        BROADCAST_STATE[user_id]["message_object"] = update.message
        BROADCAST_STATE[user_id]["status"] = "confirm_broadcast"
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "confirm_broadcast":
        ...
        if not msg:
            await query.edit_message_text("⚠️ No broadcast message found.", parse_mode="Markdown")
            return
    
        total = len(ALLOWED_USERS)
        sent = 0
        progress_message = await query.edit_message_text("🚀 Sending broadcast... (0%)")
            ...
            try:
                if msg.text:
                    ...
                    try:
                        ...
                        # If file_id expired, re-upload the document
                        try:
                            file = await context.bot.get_file(msg.document.file_id)
                            file_path = await file.download_to_drive()
                            with open(file_path, 'rb') as f:
                                await context.bot.send_document(chat_id=uid, document=f, caption=msg.caption or "")
                        except Exception as e:
                    ...
                    try:
                        ...
                        # If file_id expired, re-upload the photo
                        try:
                            file = await context.bot.get_file(msg.photo[-1].file_id)
                            file_path = await file.download_to_drive()
                            with open(file_path, 'rb') as f:
                                await context.bot.send_photo(chat_id=uid, photo=f, caption=msg.caption or "")
                        except Exception as e:
        ...
        # Ensure all tasks are executed concurrently
    ...
    if data == "cancel_broadcast":
        ...
        # Clear the broadcast state on cancel as well
        BROADCAST_STATE.pop(user_id, None)
        return
