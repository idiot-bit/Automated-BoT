...
async def ask_to_share(update: Update):
    ...
    await update.message.reply_text(
        ...
        "<b>[ SYSTEM > POST ]</b>\n"
        "<b>Status:</b> APK ready for channel\n"
        "<b>Action Required:</b> Confirm share\n\n"
        "<b>Do you want to post this to your channel?</b>\n"
        "Choose wisely: 👇",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
    ...
    if len(session_files) >= 3 and not state.get("waiting_key") and not state.get("key_prompt_sent"):
        try:
            await context.bot.edit_message_text(
                ...
                text=(
                    "🔑 <b>KEY REQUIRED</b>\n\n"
                    "<b>[ SYSTEM > KEY.REQUEST ]</b>\n"
                    "<b>APKs Received:</b> 3+\n"
                    "<b>Action Required:</b> Send your key now\n\n"
        ...
        existing_task = state.get("countdown_task")
        if existing_task and not existing_task.done():
            existing_task.cancel()
    
        # Start a new countdown and store it
...
async def method2_convert_quote(user_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Build the combined terminal-style preview as one <pre> block
    terminal_lines = [
    ...
    for idx, file in enumerate(session_files, start=1):
        ...
        quote_lines.append(f"<blockquote>Key - <code>{key}</code></blockquote>")

    preview_text = (
        "✅ <b>Key Converted to Quote Style</b>\n\n"
        f"<pre>{chr(10).join(terminal_lines)}</pre>\n"
    ...
    # Safely try to update the preview message
    try:
        await context.bot.edit_message_text(
            chat_id=user_id,
            message_id=preview_message_id,
        ...
        print(f"[ERROR] Failed to send quote preview: {e}")
...
async def method2_convert_mono(user_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Construct content inside <pre> block
    terminal_lines = [
...
async def method2_show_preview(user_id, context):
    ...
    for idx, (file_id, file_name) in enumerate(zip(session_files, session_filenames), start=1):
        ...
        try:
            ...
            size = f"{round(size_bytes / (1024 * 1024 * 1024), 2)} GB" if size_bytes >= 1024**3 else f"{round(size_bytes / (1024 * 1024), 1)} MB"
        except Exception:
            size = "?"
        ...
        # Payload preview (separate from <pre>)
    ...
    footer = (
        "\n<b>🛰️ MISSION:</b> Target channel\n"
        "<b>🧠 MODE:</b> Method 2 ~ 2 - 3 apk MethoD "
    )
