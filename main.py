...
def build_terminal_report(user_id: int, scope: str, label: str):
    ...
    joined_days = ""
    if scope == "manual":
        joined = USER_STATE.get(user_id, {}).get("joined_time")
        if joined:
            days = (datetime.now(ZoneInfo("Asia/Kolkata")) - datetime.fromtimestamp(joined, ZoneInfo("Asia/Kolkata"))).days
            joined_days = f"\n\n⏳ Joined: {days} days ago"

        all_time_apks = state.get("alltime_method1_apks", 0) + state.get("alltime_method2_apks", 0)
        all_time_keys = state.get("alltime_method1_keys", 0) + state.get("alltime_method2_keys", 0)
        joined_days += f"\n📦 All-Time: {all_time_apks} Apks, {all_time_keys} Keys"
