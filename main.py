...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # If Method 2 is active and a key is sent early (during countdown)
    if state.get("current_method") == "method2" and not state.get("waiting_key"):
        if state.get("session_files"):
            ...
            # Notify key received and show control panel
            await update.message.reply_text(
                text=(
                    f"<pre>"
                    f"▌ KEY RECEIVED ▌\n"
    ...
    # If user is prompted to send key (normal flow)
    if state.get("waiting_key") and state.get("current_method") == "method2":
        ...
        # Delete old countdown message if still visible
        if state.get("countdown_msg_id"):
            try:
                await context.bot.delete_message(
                    chat_id=user_id,
        ...
        # Send key confirmation message with clean keyboard
...
async def method2_back_fullmenu(user_id, context):
    ...
    if preview_message_id:
        try:
            ...
            # If it's because message is not modified — send new one
