...
async def schedule_stat_reports(application: Application):
    # TEMP TESTING BLOCK
    try:
        print("⏳ Sending test 6-hour report...")
        await send_user_stats_report(application.bot, hours=6)
        await reset_stats(hours=6)
        print("✅ Test report sent and stats reset.")
    except Exception as e:
        print(f"❌ Error while sending test report: {e}")

async def test_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if user_id != OWNER_ID:
        await update.message.reply_text("❌ Only the owner can run test reports.")
        return

    await send_user_stats_report(context, hours=6)
    await reset_stats(hours=6)
    await update.message.reply_text("✅ Test 6-hour report sent and stats reset.")
