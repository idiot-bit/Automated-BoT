...
async def confirm_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def send_to_user(uid):
        ...
        try:
            ...
            elif msg.document:  # Handle document (APK)
                await context.bot.send_document(
                    chat_id=uid,
            ...
            # Update progress every 5 users or when all users are sent
    ...
    tasks = [send_to_user(uid) for uid in ALLOWED_USERS]
...
async def show_broadcast_preview(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Update BROADCAST_STATE with the latest message
    ...
    if msg.text:
        # Send preview of the text message
        ...
        # Send preview of the document (APK)
        await context.bot.send_document(
            chat_id=msg.chat.id,
            document=msg.document.file_id,
        ...
        # Clear BROADCAST_STATE if unsupported
        BROADCAST_STATE.pop(user_id, None)
...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "confirm_broadcast":
        # Confirm the broadcast process
        await confirm_broadcast(update, context)
        return
    
    if data == "cancel_broadcast":
        # Cancel the broadcast and reset the state
