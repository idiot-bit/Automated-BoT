...
async def method2_show_preview(user_id, context):
    ...
    target_channel = (
        user_data.get("channel_username") or
        user_data.get("channel_id") or
        "Not set"
    )
    if str(target_channel).startswith("-100"):
        channel_display = f"ID: {target_channel}"
    elif str(target_channel).startswith("@"):
        channel_display = target_channel
    else:
        channel_display = f"@{target_channel}"
    ...
    for idx, (file_id, file_name) in enumerate(zip(session_files, session_filenames), start=1):
        ...
        size_display = f"{round(file_size / (1024 * 1024), 2)} MB" if file_size else "?"
        apk_lines.append(f"{idx}. {file_name} [{size_display}]")

    apk_text = "\n".join(apk_lines)

    # Key formatting
    if key_mode == "quote":
        styled_key = f"<blockquote>Key - <code>{key}</code></blockquote>"
    elif key_mode == "mono":
        styled_key = f"Key - <code>{key}</code>"
    else:
        styled_key = f"Key - {key}"

    # Full message content
    preview_text = (
        f"<b>[ PREVIEW INFO ]</b>\n\n"
        f"👤 <b>User ID:</b> <code>{user_id}</code>\n"
        f"📡 <b>Target Channel:</b> {channel_display}\n\n"
        f"{apk_text}\n\n"
        f"{styled_key}"
    )
    ...
    try:
        if message_id:
            ...
            USER_STATE[user_id]["preview_message_id"] = sent_msg.message_id
    except Exception as e:
