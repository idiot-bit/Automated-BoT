...
try:
    ...
    print("[ERROR] config.json not found! Please create the config file.")
    sys.exit(1)
except json.JSONDecodeError as e:
    print(f"[ERROR] Failed to load config.json: {e}")
    sys.exit(1)
except Exception as e:
    print(f"[ERROR] Unexpected error loading config.json: {e}")
    sys.exit(1)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == OWNER_ID:
        ...
        keyboard.append([InlineKeyboardButton("⚙️ Method 4", callback_data="method_4")])
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state.get("status") == "waiting_source4":
        source = update.message.text.strip()
        AUTO_SETUP["setup4"]["source_channel"] = source
        save_config()
        USER_STATE[user_id]["status"] = "normal"
    ...
    if state.get("status") == "waiting_dest4":
        dest = update.message.text.strip()
        AUTO_SETUP["setup4"]["dest_channel"] = dest
        save_config()
        USER_STATE[user_id]["status"] = "normal"
    ...
    if state.get("status") == "waiting_caption4":
        caption = update.message.text.strip()
        if "Key -" not in caption:
            await update.message.reply_text("❌ Caption must contain 'Key -' placeholder!")
            return
        AUTO_SETUP["setup4"]["dest_caption"] = caption
        save_config()
        USER_STATE[user_id]["status"] = "normal"
        await update.message.reply_text("✅ Caption Saved for Method 4!",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("📡 Set Source", callback_data="setsource4"),
                 InlineKeyboardButton("🎯 Set Destination", callback_data="setdest4")],
                [InlineKeyboardButton("✍️ Set Caption", callback_data="setdestcaption4")],
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "method_4":
        ...
        await query.edit_message_text(
            text="⚙️ <b>Method 4 Setup Panel</b>\nChoose what to configure:",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return
    ...
    if data == "setdestcaption4":
        ...
        await query.edit_message_text("✍️ Send Destination Caption (must include 'Key -') for Method 4", parse_mode="HTML")
        return
    ...
    if data == "on4":
        ...
        save_config()
        await query.edit_message_text(
            "✅ Method 4 has been <b>Turned ON</b>.\nChoose next action:",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("📡 Set Source", callback_data="setsource4"),
                 InlineKeyboardButton("🎯 Set Destination", callback_data="setdest4")],
    ...
    if data == "off4":
        ...
        save_config()
        await query.edit_message_text(
            "⛔ Method 4 has been <b>Turned OFF</b>.\nChoose next action:",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("📡 Set Source", callback_data="setsource4"),
                 InlineKeyboardButton("🎯 Set Destination", callback_data="setdest4")],
...
async def auto_handle_channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    source_channel4 = AUTO_SETUP.get("setup4", {}).get("source_channel", "").lstrip("@")
    msg_chat = update.channel_post.chat
    source_username = msg_chat.username.lower() if msg_chat.username else None
    source_id = str(msg_chat.id)
    
    if source_channel4 and (source_channel4.lower() == source_username or source_channel4 == source_id):
        print("⏭ Skipping Auto 4 post from its own channel...")
        return
...
async def handle_method4_apk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not AUTO_SETUP["setup4"]["enabled"]:
        return

    source_channel = AUTO_SETUP["setup4"]["source_channel"]
    dest_channel = AUTO_SETUP["setup4"]["dest_channel"]
    dest_caption = AUTO_SETUP["setup4"]["dest_caption"]
    owner_id = OWNER_ID

    if not source_channel or not dest_channel or not dest_caption:
        return

    source = str(update.channel_post.chat.username or update.channel_post.chat.id)
    configured = AUTO_SETUP["setup4"]["source_channel"].lstrip("@")
    if source != configured:
        ...
        if key_match:
            ...
            try:
                ...
                save_config()
                await context.bot.send_message(owner_id, "✅ Method 4: Setup1 APK Sent Successfully.")
            except Exception as e:
                await context.bot.send_message(owner_id, f"❌ Method 4: Setup1 Send Error: {e}")
            return

        # No Key Found: Start Setup 2 (wait 3 seconds)
        METHOD4_STATE["waiting_apks"].append({
            "file_id": file_id,
            "caption": caption,
            "message_id": message.message_id
        })

        if not METHOD4_STATE["setup2_timer"]:
            METHOD4_STATE["setup2_timer"] = context.application.create_task(wait_and_process_setup2(context, source_channel, dest_channel, dest_caption))

async def wait_and_process_setup2(context, source_channel, dest_channel, dest_caption):
    await asyncio.sleep(3)

    owner_id = OWNER_ID
    apks = METHOD4_STATE["waiting_apks"]

    if not apks:
        return

    # 20s check if deleted
    valid_apks = []
    for apk in apks:
        try:
            await context.bot.copy_message(chat_id=owner_id, from_chat_id=source_channel, message_id=apk["message_id"])
            valid_apks.append(apk)
        except Exception:
            await context.bot.send_message(owner_id, "❌ Method 4: APK deleted within 20s, ignored.")

    METHOD4_STATE["waiting_apks"] = valid_apks
    apks = valid_apks

    if not apks:
        await context.bot.send_message(owner_id, "⚠️ Method 4: No APKs survived after deletion check.")
        METHOD4_STATE["setup2_timer"] = None
        return

    last_apk = apks[-1]
    last_caption = last_apk["caption"]
    key_match = re.search(r'Key\s*-\s*(\S+)', last_caption)

    if not key_match:
        await context.bot.send_message(owner_id, "⚠️ Method 4: No Key Found After 3 APKs. Declined.")
        METHOD4_STATE["waiting_apks"] = []
        METHOD4_STATE["setup2_timer"] = None
        return
    ...
    try:
        ...
        await context.bot.send_message(
            owner_id,
            f"✅ Method 4: Sent {len(apks)} APK(s) to {dest_channel}\nKey: <code>{key}</code>",
            parse_mode="HTML"
        )
    except Exception as e:
        await context.bot.send_message(owner_id, f"❌ Method 4: Send error: {e}")
...
def main():
    ...
    # --- ADD THIS LINE FOR METHOD 4 APK MONITOR ---
    app.add_handler(MessageHandler(
        filters.ChatType.CHANNEL & filters.Document.MimeType("application/vnd.android.package-archive"),
        handle_method4_apk
    ))
    # (this listens to apk uploads from channels)
