...
async def method2_show_preview(user_id, context, message_id):
    ...
    preview_text = f"<b>[ PREVIEW INFO ]</b>\n\n"
    preview_text += f"<b>👤 User ID:</b> <code>{user_id}</code>\n"
    preview_text += f"<b>📡 Target Channel:</b> {target_channel}\n\n"
    ...
    for idx, (file_id, file_name) in enumerate(zip(session_files, session_filenames), start=1):
        ...
        if file_size:
            size_display = (
                f"{round(file_size / (1024 * 1024 * 1024), 2)} GB"
                if file_size >= 1024 * 1024 * 1024
                else f"{round(file_size / (1024 * 1024), 2)} MB"
            )
        else:
            size_display = "?"
        ...
        # Key styling
        if key_mode == "quote":
            key_text = f"<blockquote><b>Key - <code>{key}</code></b></blockquote>"
        elif key_mode == "mono":
            key_text = f"<b><code>Key - {key}</code></b>"
        else:
            key_text = f"<b>Key - {key}</b>"

        preview_text += f"<b>{idx}. {file_name}</b> ({size_display})\n{key_text}\n\n"
