...
BROADCAST_STATE = {}  # Track if owner is in broadcast mode
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == OWNER_ID and BROADCAST_STATE.get(user_id, {}).get("status") == "waiting_message":
...
async def countdown_and_check(user_id, chat_id, context):
    try:
        for remaining in range(5, 0, -1):
            ...
            # After 5 sec, check session
            state = USER_STATE.get(user_id, {})
            if state and state.get("session_files") and not state.get("waiting_key", False):
                await ask_key_for_method2(user_id, context)
...
async def confirm_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    msg = BROADCAST_STATE.get(user_id, {}).get("message_object")

    if not msg:
        await update.callback_query.edit_message_text("⚠️ No broadcast message found.", parse_mode="Markdown")
        return

    total = len(ALLOWED_USERS)
    sent = 0
    progress_message = await update.callback_query.edit_message_text("🚀 Sending broadcast... (0%)")
    ...
    async def send_to_user(uid):
        ...
        try:
            ...
            elif msg.document:  # Handle APKs or any document
                try:
                    await context.bot.send_document(
                        chat_id=uid,
                        document=msg.document.file_id,
                        caption=msg.caption or "Broadcast Message"
                    )
                except Exception:
                    try:
                        ...
                        with open(file_path, 'rb') as f:
                            await context.bot.send_document(chat_id=uid, document=f, caption=msg.caption or "Broadcast Message")
                    except Exception as e:
                        print(f"Document reupload failed for {uid}: {e}")

            elif msg.photo:  # Handle images
                try:
                    await context.bot.send_photo(
                        chat_id=uid,
                        photo=msg.photo[-1].file_id,
                        caption=msg.caption or "Broadcast Photo"
                    )
                except Exception:
                    try:
                        ...
                        file_path = await file.download_to_drive()
                        with open(file_path, 'rb') as f:
                            await context.bot.send_photo(chat_id=uid, photo=f, caption=msg.caption or "Broadcast Photo")
                    except Exception as e:
                        print(f"Photo reupload failed for {uid}: {e}")
            ...
            if sent % 5 == 0 or sent == total:
                ...
                try:
                    await progress_message.edit_text(f"🚀 Sending broadcast... ({percent}%)")
                except Exception as e:
    ...
    try:
        ...
        print(f"Final progress edit failed: {e}")
        pass

    # Clear the broadcast state after completion
    BROADCAST_STATE.pop(user_id, None)

async def show_broadcast_preview(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    BROADCAST_STATE[user_id] = {"status": "waiting_message", "message_object": update.message}
    ...
    if msg.text:
        ...
        await context.bot.send_document(
            chat_id=msg.chat.id,
            document=msg.document.file_id,
            caption="🗒️ Broadcast Preview\nSend this document to all users?",
            parse_mode="HTML",
            reply_markup=keyboard
        )
        ...
        await msg.reply_text(
            "⚠️ Unsupported message type for broadcast!",
            parse_mode="HTML"
        )
        BROADCAST_STATE.pop(user_id, None)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- BROADCAST HANDLING BEFORE ANYTHING ---
    if user_id == OWNER_ID and BROADCAST_STATE.get(user_id, {}).get("status") == "waiting_message":
        await show_broadcast_preview(update, context)
        return
    ...
    if message_text == "ping":
        ...
        BROADCAST_STATE[user_id] = {"status": "waiting_message"}
        await update.message.reply_text(
            "📝 <b>Broadcast Mode Activated!</b>\n\nType the message you want to send to all allowed users:",
            parse_mode="HTML"
        )
        return
...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = query.data  # Get the callback data
