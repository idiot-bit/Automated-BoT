...
async def ask_to_share(update: Update):
    ...
    channel = pending.get("channel", "Not Set")
    ...
    preview = (
        "📤 <b>Share Confirmation</b>\n\n"
        "<pre>[ SYSTEM > POST.READY ]\n"
        f"[🔑] Key     : {key}\n"
        f"[📡] Channel : {channel}\n"
        f"[👤] User ID : {user_id}\n"
        "[✅] Status  : Ready to post</pre>\n\n"
...
async def process_method2_apk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Update or send progress message (UPDATED STYLE)
    text = (
        f"✅ <b>{len(session_files)} APK(s) Received</b>\n"
        f"⏳ <i>Waiting 5 seconds for more files...</i>"
    )
    if progress_id:
        try:
            await context.bot.edit_message_text(
    ...
    # Prompt for key if 3+ APKs received (UPDATED STYLE)
    if len(session_files) >= 3 and not state.get("waiting_key") and not state.get("key_prompt_sent"):
        key_request_text = (
            "🔑 <b>Key Required</b>\n\n"
            f"📦 You’ve uploaded <b>{len(session_files)}</b> APKs.\n"
            "Please send your <b>Key</b> now to continue.\n\n"
            "<i>Note: One key is valid for 2–3 APKs.</i>"
        )
        try:
            await context.bot.edit_message_text(
                chat_id=chat_id,
...
async def auto_recaption(user_id, context):
    ...
    try:
        await context.bot.edit_message_text(
            ...
            text=(
                "✅ <b>RECAPTION COMPLETE</b>\n"
                "━━━━━━━━━━━━━━━━━━━━━\n"
                "<pre>"
                "[✔] APKs Updated  : {0}\n"
                "[✔] Key Injected  : Yes\n"
                "[✔] Style Applied : Final APK\n"
                "[✔] Status        : Success\n"
                "━━━━━━━━━━━━━━━━━━━━━━"
                "</pre>\n"
                "<b>Ready:</b> View post or continue ops below."
            ).format(len(session_files)),
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(buttons)
        )
    except Exception as e:
...
async def auto_last_caption(user_id, context):
    ...
    await context.bot.edit_message_text(
        ...
        message_id=preview_message_id,
        text=(
            "✅ <b>RECAPTION MODE: LAST APK</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━\n"
            "<pre>"
            "[✔] Style Injected  : Final APK\n"
            "[🚀] Media Reposted : Yes\n"
            "[🔗] Post Status    : Linked\n"
            "[🧠] Session        : Closed\n"
            "━━━━━━━━━━━━━━━━━━━━━"
            "</pre>\n"
            "<b>Next:</b> View post or continue with another method."
        ),
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(buttons)
    )
...
async def last_caption_key(user_id, context):
    ...
    # Update preview with terminal-style formatting
    await context.bot.edit_message_text(
        chat_id=user_id,
        message_id=preview_message_id,
        text=(
            "✅ <b>RECAPTION MODE: LAST KEY ONLY</b>\n"
            "━━━━━━━━━━━━━━━━━━━━━\n"
            "<pre>"
            "[✔] Style Injected  : Final APK Only\n"
            "[🔑] Key Format     : {0}\n"
            "[📄] Post Status    : Linked\n"
            "[🧠] Session        : Closed\n"
            "━━━━━━━━━━━━━━━━━━━━━"
            "</pre>\n"
            "<b>Next:</b> View post or select a new method."
                ...
                try:
                    ...
                    if "Message is not modified" in str(e):
                        ...
                        button = InlineKeyboardMarkup([
                            [InlineKeyboardButton("🧹 Start New Clear all", callback_data="fresh_session")]
                        ])
                        await context.bot.send_message(
                            chat_id=chat_id,
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state.get("waiting_key") and state.get("current_method") == "method2":
        ...
        if not key or not session_files:
            await update.message.reply_text(
                "❌ <b>Session Error! Please restart.</b>",
                parse_mode="HTML"
            )
            return
    
        # Save key and reset states
        ...
        # Pull user + caption data
        user = update.effective_user
        user_first = user.first_name or "User"
        channel_id = state.get("channel_id", "Not Set")
        caption_template = USER_DATA.get(str(user_id), {}).get("caption", "Key -")
        final_caption = caption_template.replace("Key -", f"Key - {key}")
    
        # Terminal-style preview
        text = (
            "<b>🧠 SYSTEM.ACTIVATE > KEY RECEIVED</b>\n"
            "<pre>"
            f"[👤] User        : {user_first} ({user_id})\n"
            f"[🔑] Key         : {key}\n"
            ...
            "────────────────────────────────────"
            "</pre>\n"
            "<b>Status:</b> Key applied. Choose your next action below."
        )
...
async def method2_convert_quote(user_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Inject key into user's saved caption
    user_data = USER_DATA.get(str(user_id), {})
    caption_template = user_data.get("caption", "Key -")
    final_caption = caption_template.replace("Key -", f"Key - {key}")

    # Build styled quote output
    text = (
        "✅ <b>Key converted to Quote Style!</b>\n\n"
        f"<b>Style:</b> {key_mode}\n"
        "<pre>"
        f"{final_caption}"
        "</pre>"
    )
...
async def method2_convert_mono(user_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    key = state.get("saved_key", "")
    session_files = state.get("session_files", [])
    key_mode = state.get("key_mode", "normal").capitalize()
    ...
    # Fetch user's saved caption template and inject key
    user_data = USER_DATA.get(str(user_id), {})
    caption_template = user_data.get("caption", "Key -")
    final_caption = caption_template.replace("Key -", f"Key - {key}")
    ...
    text = (
        "✅ <b>Key converted to Mono Style!</b>\n\n"
        f"<b>Style:</b> {key_mode}\n"
        "<pre>"
        f"{final_caption}"
        "</pre>"
...
async def method2_edit_caption(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "Key -" not in new_caption:
        await update.message.reply_text(
            "❌ <b>Invalid Caption!</b>\n\n"
            "Your caption must include the placeholder: <code>Key -</code>",
            parse_mode="HTML"
        )
        return
    ...
    if not preview_message_id or not key or not session_files:
        await update.message.reply_text(
            ...
            "Please start <b>Method 2</b> again.",
            parse_mode="HTML"
        )
        return

    # Inject key into caption
    full_caption = new_caption.replace("Key -", f"Key - {key}")

    text = (
        "✅ <b>Caption Updated</b>\n\n"
        f"<pre>{full_caption}</pre>"
    )
...
async def method2_back_fullmenu(user_id, context):
    ...
    # Inject real key into caption
    caption_template = USER_DATA.get(str(user_id), {}).get("caption", "Key -")
    final_caption = caption_template.replace("Key -", f"Key - {key}")

    # Hacker preview
    text = (
        "<b>🧠 SYSTEM.RESTORE > FULL SESSION</b>\n"
        "<pre>"
        f"[👤] User        : {user_first} ({user_id})\n"
        f"[🔑] Key         : {key}\n"
        f"[📡] Channel ID  : {channel_id}\n"
        ...
        "────────────────────────────────────"
        "</pre>\n"
        "<b>Status:</b> Session active. Choose your next action below."
    )
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton("📝 New Caption", callback_data="method2_edit"),
            InlineKeyboardButton("👁️ Channel Output", callback_data="method2_preview")
        ],
        [
            InlineKeyboardButton("🧹 Clear all Start New", callback_data="erase_all")
        ]
    ]
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "method2_back_fullmenu":
        ...
        key = state.get("saved_key", "")
        session_files = state.get("session_files", [])
        user_first = update.effective_user.first_name or "User"
        channel_id = state.get("channel_id", "Not Set")
        ...
        text = (
            ...
            "────────────────────────────────────"
            "</pre>\n"
            "<b>Status:</b> Session restored. Continue below."
        )
...
async def auto_handle_channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Initial waiting message with full empty bar
    countdown_msg = await context.bot.send_message(
        chat_id=OWNER_ID,
        text=(
            f"⏳ <b>Auto {setup_number} - Waiting...</b>\n"
            "<code>[██████████░░░░░░░░░░] (0/20)</code>"
        ),
        parse_mode="HTML"
    )
