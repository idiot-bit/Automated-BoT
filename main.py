...
APK_COLLECTION = {} # Global cache to store incoming APKs temporarily
...
async def collect_apks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.channel_post:
        return

    message = update.channel_post
    chat_id = message.chat.id
    media_group_id = message.media_group_id or message.message_id  # Use media_group_id if exists else message_id

    # Initialize if not exist
    if chat_id not in APK_COLLECTION:
        APK_COLLECTION[chat_id] = {}

    if media_group_id not in APK_COLLECTION[chat_id]:
        APK_COLLECTION[chat_id][media_group_id] = []

    # Append current message
    APK_COLLECTION[chat_id][media_group_id].append(message)

    # Wait 3 seconds to collect
    await asyncio.sleep(3)

    apks = APK_COLLECTION[chat_id].pop(media_group_id, [])

    if not (1 <= len(apks) <= 3):
        print(f"❌ Invalid number of APKs: {len(apks)}. Skipping...")
        return

    print(f"✅ Collected {len(apks)} APK(s). Starting 20s deletion check...")

    await start_20s_wait_and_process(chat_id, apks, context)

async def start_20s_wait_and_process(chat_id, apks, context):
    owner_wait_message = await context.bot.send_message(
        chat_id=OWNER_ID,
        text="⏳ Waiting 20 seconds to confirm APKs are stable...",
        parse_mode="Markdown"
    )
    ...
    for sec in range(19, 0, -1):
        ...
        try:
            await context.bot.edit_message_text(
                chat_id=OWNER_ID,
                message_id=owner_wait_message.message_id,
                text=f"⏳ {sec}s remaining for APK stability check...",
                parse_mode="Markdown"
            )
        except:
            pass
    ...
    # Now check if messages still exist
    for message in apks:
        try:
            await context.bot.forward_message(chat_id=OWNER_ID, from_chat_id=chat_id, message_id=message.message_id)
        except:
            await context.bot.edit_message_text(
                chat_id=OWNER_ID,
                message_id=owner_wait_message.message_id,
                text="❌ Some APK(s) were deleted. Skipping process.",
                parse_mode="Markdown"
            )
            print("❌ Deletion detected. Aborting.")
            return

    print("✅ All APKs stable. Proceeding to key extraction and sending...")

    await process_apks(apks, context)

async def process_apks(apks, context):
    key = None

    # Try to find key from 1st → 2nd → 3rd
    for message in apks:
        if message.caption:
            match = re.search(r'Key\s*-\s*(\S+)', message.caption)
            if match:
                key = match.group(1)
                break
    ...
    if not key:
        await context.bot.send_message(
            chat_id=OWNER_ID,
            text="❌ No Key found in any caption. Skipping.",
            parse_mode="Markdown"
        )
        print("❌ No key found. Skipping process.")
        return

    print(f"✅ Key found: {key}")

    # Fetch destination setups
    dest1 = AUTO_SETUP["setup1"].get("dest_channel", "")
    dest2 = AUTO_SETUP["setup2"].get("dest_channel", "")
    dest3 = AUTO_SETUP["setup3"].get("dest_channel", "")

    caption1 = AUTO_SETUP["setup1"].get("dest_caption", "")
    caption2 = AUTO_SETUP["setup2"].get("dest_caption", "")
    caption3 = AUTO_SETUP["setup3"].get("dest_caption", "")
    ...
    # Helper for building final caption
    ...
    for message in apks:
        ...
        try:
            if 1 <= file_size <= 50:
                caption = build_caption(caption1, key, style1)
                ...
                sent_links.append(f"Auto1: https://t.me/{dest1.strip('@')}/{sent.message_id}")

            elif 80 <= file_size <= 2048:
                caption = build_caption(caption2, key, style2)
                sent = await context.bot.send_document(
                    chat_id=dest2,
                    document=doc.file_id,
                    caption=caption,
                ...
                sent_links.append(f"Auto2: https://t.me/{dest2.strip('@')}/{sent.message_id}")

            else:
                caption = build_caption(caption3, key, style3)
                sent = await context.bot.send_document(
                    chat_id=dest3,
                    document=doc.file_id,
                    caption=caption,
                ...
                sent_links.append(f"Auto3: https://t.me/{dest3.strip('@')}/{sent.message_id}")

        except Exception as e:
            print(f"❌ Error sending APK: {e}")

    # Send success quietly to owner
    if sent_links:
        text = "✅ Completed:\n\n" + "\n".join([f"🔗 {link}" for link in sent_links])
        await context.bot.send_message(chat_id=OWNER_ID, text=text, parse_mode="Markdown")
        print("✅ Sent success message to owner.")
...
def main():
    ...
    app.add_handler(MessageHandler(filters.ChatType.CHANNEL, collect_apks))
