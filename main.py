    ...
    try:
        ...
        if user_id == OWNER_ID and BROADCAST_SESSION.get(user_id, {}).get("waiting_for_message"):
            ...
            await context.bot.send_photo(
                ...
                caption=msg.caption or "🖼️ Image received.",
                parse_mode="HTML",
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # --- Save user info if new (for broadcast) ---
        if str(user_id) not in USER_DATA:
            USER_DATA[str(user_id)] = {
                "first_name": user.first_name,
                "username": user.username,
        ...
        if user_id == OWNER_ID and BROADCAST_SESSION.get(user_id, {}).get("waiting_for_message"):
            ...
            await context.bot.send_document(
                ...
                caption=msg.caption or "📄 Document received.",
                parse_mode="HTML",
                reply_markup=preview_keyboard
            )
            return
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # --- New Broadcast Text Receiving (Owner Only) ---
        if user_id == OWNER_ID and BROADCAST_SESSION.get(user_id, {}).get("waiting_for_message"):
            msg = update.message
...
async def send_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        for uid in user_ids:
            ...
            try:
                if msg.text:
                    await context.bot.send_message(chat_id=uid, text=msg.text, parse_mode="HTML", reply_markup=keyboard)
                elif msg.photo:
                    await context.bot.send_photo(chat_id=uid, photo=msg.photo[-1].file_id, caption=msg.caption, parse_mode="HTML", reply_markup=keyboard)
                elif msg.document:
                    await context.bot.send_document(chat_id=uid, document=msg.document.file_id, caption=msg.caption, parse_mode="HTML", reply_markup=keyboard)
                elif msg.video:
                    await context.bot.send_video(chat_id=uid, video=msg.video.file_id, caption=msg.caption, parse_mode="HTML", reply_markup=keyboard)
                else:
                    continue
