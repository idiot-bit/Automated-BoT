...
async def backup_config(context=None):
    now = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M")
    config_path = "config.json"
    zip_filename = f"bot_backup_{now}.zip"
    ...
    # Create ZIP file with 4 required files
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "backup_config":
        await backup_config(context)
        await query.edit_message_text("✅ Full backup ZIP created and sent to your PM!")
        return
