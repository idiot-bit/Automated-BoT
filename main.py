...
async def process_method1_apk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Fallback: if no regex match, try to extract from 'code' caption entity
    if not key and update.message.caption_entities:
        ...
        # Store everything needed temporarily in state
...
async def handle_settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "view_users":
        ...
        message = await query.edit_message_text(
            "📁 Please upload your backup ZIP file now.\nOnly valid `.zip` files will be accepted."
        )
        ...
        async def cancel_zip_restore():
            ...
            state = USER_STATE.get(user_id, {})
            if state.get("awaiting_zip") and time.time() >= state.get("zip_timeout", 0):
                ...
                try:
                    await context.bot.delete_message(chat_id=chat_id, message_id=state.get("zip_prompt_message_id"))
                except:
                    pass
    
                # Send timeout notice + settings panel
                await context.bot.send_message(
                    chat_id=user_id,
                    text="⏳ Backup restore timed out. Please try again.",
                    parse_mode="HTML",
                    reply_markup=get_settings_keyboard()
                )
        ...
        USER_STATE.setdefault(user_id, {})
        USER_STATE[user_id]["status"] = "awaiting_add_user"
        await query.edit_message_text("✏️ Send the user ID to *add*", parse_mode="Markdown")
        return
    
    elif data == "remove_user":
        USER_STATE.setdefault(user_id, {})
        USER_STATE[user_id]["status"] = "awaiting_remove_user"
        await query.edit_message_text("✏️ Send the user ID to *remove*", parse_mode="Markdown")
        return
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Admin link input
    if state.get("awaiting_admin_link"):
        USER_STATE.setdefault(user_id, {})
        if message_text.startswith("https://"):
            global BOT_ADMIN_LINK
            BOT_ADMIN_LINK = message_text
            config["bot_admin_link"] = message_text
            save_config()
    ...
    # Add user
    if status == "awaiting_add_user":
        try:
            new_uid = int(message_text)
            ALLOWED_USERS.add(new_uid)
            save_config()
            await update.message.reply_text(
                f"✅ User <code>{new_uid}</code> added.",
                parse_mode="HTML",
                reply_markup=get_settings_keyboard()
            )
        except:
    ...
    # Remove user
    elif status == "awaiting_remove_user":
        try:
            rem_uid = int(message_text)
            if rem_uid in ALLOWED_USERS:
                ALLOWED_USERS.remove(rem_uid)
                save_config()
                await update.message.reply_text(
                    f"🗑️ User <code>{rem_uid}</code> removed.",
                    parse_mode="HTML",
                    reply_markup=get_settings_keyboard()
                )
            else:
                await update.message.reply_text("⚠️ That user is not in the list.")
        except:
            await update.message.reply_text("❌ Invalid user ID.")

    if status in ["awaiting_add_user", "awaiting_remove_user"]:
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "share_yes":
        ...
        await query.edit_message_text("✅ Sending APK...")
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(
        ...
        pattern=r"^(view_users|view_autosetup|viewsetup[1-4]|backup_config|force_reset|confirm_reset|settings_back|bot_admin_link|backup_restore|cancel_restore|confirm_restore)$"
    ))
