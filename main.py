...
async def send_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for idx, uid in enumerate(user_ids, 1):
        ...
        try:
            ...
            sent += 1
            sent_users.append(
                f"• <b>{name}</b> | @{uname or '—'} | <code>{uid}</code> | ✅ Active"
            )

        except Forbidden:
            failed += 1
            failed_users.append(
                f"• <b>{name}</b> | @{uname or '—'} | <code>{uid}</code> | ❌ Blocked"
            )
        except Exception:
            failed += 1
            failed_users.append(
                f"• <b>{name}</b> | @{uname or '—'} | <code>{uid}</code> | ⚠️ Error"
            )
    ...
    try:
        ...
        print(f"Failed to edit summary: {e}")
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(
        ...
        pattern=r"^(view_users|view_autosetup|viewsetup[1-4]|backup_config|force_reset|confirm_reset|settings_back|back_to_methods)$"
    ))
