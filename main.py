...
async def method2_show_preview(user_id, context):
    ...
    # Terminal-style session preview
    terminal_block = f"┌───────────────[ PREVIEW INFO ]───────────────┐\n"
    terminal_block += f"│ User ID        : {user_id}\n"
    terminal_block += f"│ Target Channel : {target_channel}\n"
    terminal_block += f"├──────────────────────────────────────────────┤\n"
    ...
    for idx, (file_id, file_name) in enumerate(zip(session_files, session_filenames), start=1):
        ...
        terminal_block += f"│ {idx}. {file_name} ({size_display})\n"
